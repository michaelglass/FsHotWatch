...
/// false. So the cause is carried, not re-guessed at the terminal.
///
...
/// DID A TEST HOST DIE? The `BuildWait`, and deliberately its twin:
...
/// What a green is RELATIVE to. A `Clean` — and so a `Verdict.Green` —
...
[<RequireQualifiedAccess>]
type CheckOutcome =
    /// Complete coverage and no failures — relative to `baseline`.
    | Clean of baseline: Baseline
    /// No failures, complete coverage, an acceptable scope — and NO
    ...
    /// The daemon SETTLED — it built, it ran the suite, it committed
...
let exitCode (outcome: CheckOutcome) : int =
    ...
    // No verdict, for the unearned-scope reason: what ran passed, and
...
type CheckInputs =
    {
        ...
        Scope: TestScope
        /// What the daemon said its greens are relative to. `NotReported`
...
let verdict (mode: CheckMode) (inputs: CheckInputs) : CheckOutcome =
    ...
    // Checked BEFORE `FailuresFound` and nowhere else, because it is a
    ...
    if CheckInputs.onlyUnattributableFailures inputs then
        ...
        // No real failure, but a test host was KILLED mid-run, so its
        ...
        // The only door to `Clean`: every arm below that used to be
        ...
        | Complete ->
            ...
            // exit 0. Same principle as the unreadable ledger: a MISSING
