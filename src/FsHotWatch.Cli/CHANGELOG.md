...
- feat!: **`fshw gate --run-once` — the merge verdict, without a daemon.**
...
- feat!: **the gate RUNS the suite it demands.** `set-scope full` makes
...
- **Note on "full suite":** the gate asserts that every test project **`.fshw.json` knows
  ...
  is in the solution but not in `.fshw.json`, so the gate does not run it.
...
- feat!: **the verdict is a FILE, content-addressed to the tree it verified.** (ADR-013) Every `check` and `gate` now publishes `.fshw/verdict.json` — written
...
- fix!: **`fshw gate` could never go green.** `readTestScope` and
...
- fix: parse the `RunVerdict` (summary + elapsed) the daemon now sends on
  `completed` statuses; payloads from older daemons parse to an
