...
let pluginOutcomeOf (warningsAreFailures: bool) (now: DateTime) (parsed: ParsedPluginStatus) : PluginOutcome option =
    ...
    //   * a run record whose outcome is `VerifiedNothing` — the run
...
/// One configured hook step which ACTUALLY EXECUTED during this
...
/// One measured half-open interval `[StartOffsetMs, StartOffsetMs +
...
/// Everything a verdict says about WHERE its wall time went, over and
...
/// produced.
///
/// A single check runs the tests SEVERAL times: the impact-selected
...
module RedCause =
    ...
    /// happens to quote the phrase is not the compiler crashing. Every failing
    /// `fcs-internal` entry is a checker fault by its source alone: the daemon writes
    /// nothing else there.
    [<Literal>]
...
type Outcome =
    /// Green RELATIVE TO a baseline — the full-suite run every test
...
module CheckProse =
    ...
    /// A test HOST that DIED, in its own words.
    ///
    ...
    /// What ran passed, and nothing vouches for what did not run. The
    ...
    /// each had to add their arms twice. That is the
    ...
    let explainOutcome (outcome: CheckVerdict.CheckOutcome) : string option =
        ...
        // The stale-output cause, in its own words: retrying is the one
        ...
        // A killed host, in its own words — never `waitingOnBuild`'s
        ...
        // Its own words, because the usual reading of an exit 3 — "the
...
let outcomeOfCheck (outcome: CheckVerdict.CheckOutcome) : Outcome =
    ...
    // `incomplete`, never `green`, and the reason names the missing
    ...
    // `incomplete`, never `red`. This is the machine-readable half of
    ...
    // `incomplete`, never `red`: the structured outcome is what a deploy
    ...
    // `incomplete` in the FILE, and exit 7 beside it — the two are not
...
type Verdict =
    private
        {
            ...
            treeFileCount: int
            /// How many of `treeFileCount` came from
            ...
            scope: TestScope
            /// The test projects in the solution that this run
            ...
            /// EVERY test run this check produced, and the reports each
            ...
            /// The failing ledger diagnostics this verdict's exit code
            ...
            triggerCount: int
            /// Hook steps, interval evidence, observed wall time and
    ...
    ///
    /// It is the graded run, NOT the whole of what this check executed:
    ...
    /// Files hashed because the repo DECLARED them — see the
    /// `treeDeclaredCount` field.
    member this.TreeDeclaredCount = this.treeDeclaredCount

    /// Declarations that matched no file and contributed a sentinel.
    ...
    /// Every batch this check ran, graded run first. A check that ran
    ...
    /// How the impact-scoped reading `confirm` escalated away from compared
    ...
    /// The failing ledger diagnostics behind a red exit code — including
    ...
    /// Configured hook steps that executed in this invocation.
...
/// The baseline a green names must agree with the scope beside it:
...
let private validate (v: Verdict) : Result<Verdict, string> =
    ...
    // The classification and the reading it classified are ONE fact split
...
let create
    ...
    (tree: TreeHash.Tree)
    // The declared, reasoned gaps in this run's scope. REQUIRED
    ...
    (exitCode: int)
    (plugins: PluginVerdict list)
    // EVERY batch this check ran, graded run first — see `RunSuites`
    ...
    (runs: RunSuites list)
    (comparison: CheckComparison)
    // EVERY failing ledger diagnostic the exit code was computed from,
...
/// The same verdict, carrying its attribution. Separate from `create`
...
/// The declared gaps, on the wire, INSIDE `scope`.
///
...
let serialize (v: Verdict) : string =
    let payload =
        {| schema = Schema
                  ...
                  {| name = p.Name
                     ...
                     summary =
                      (match p.Summary with
                       | Some s -> box s
                       | None -> null) |} ]
           // EVERY batch this check ran, with the reports each wrote.
           ...
           // What reddened this verdict, when the answer is not in
           ...
           // Work outside the plugin run records, on one timeline.
           hooks =
            [ for h in v.Hooks ->
                  {| scope = h.Scope
...
/// Every write to the verdict file — the publisher's, the wrapper's
...
// ---------------------------------------------------------------------------
// Attaching evidence to a verdict ANOTHER process wrote.
//
                ...
                | Some treeHash, Some outcome, Ok plugins, Ok runs, Some projectModel ->
                    ...
                    let rehydrated =
                        { producedAt =
                          ...
                          // Absent in verdicts written. Zero is
                          ...
                          // Absent in verdicts written before the field
...
type Applicability =
    ...
    /// Its `treeHash` was computed by a DIFFERENT hashing scheme, so
    ...
    /// Everything above matched — this verdict describes the tree on
...
/// verdict has always recorded `treeHashAlgorithm` and never read it:
...
/// `Applies`. That ordering is load-bearing in both directions: every
...
type Report =
    ...
    /// A run is in flight over THIS tree, and this verdict is the
...
///
/// A run being IN FLIGHT does not disqualify the prior verdict here,
...
let describeStillApplies (v: Verdict) : string =
    ...
    let evidence =
        ...
        | suites ->
            ...
            // Said out loud when the check ran the tests more than once,
    ...
    // The baseline is what makes this green auditable as a claim about
...
/// EVERY batch this check has evidence from, each with the reports it
...
/// Turn what a transport observed just BEFORE `confirm` escalated into the
        ...
        | Some _, Some _ ->
            ...
            | ReachedNoFailure missed ->
                ...
                | Red ->
                    ...
                    | NoneBeyondTheTests ->
                        // `check` would have been green RELATIVE to the
...
/// What a transport can OFFER for the check-vs-confirm sample. Three
