...
/// next `check` stamp a content-free green. The act of measuring
...
type PluginOutcome =
    ...
    ///Its OWN token, because a wedge read as mere "running" is how
...
/// surface and `wedged` on the other. Both of the fail-closed rules the tracked issue
...
///     ✓ is the trap exactly (an operator had to diagnose a wedge from a
...
/// to `0L` when no measurement existed — which is the signature
...
/// The tracked issue made for `RunVerdict` (private record, `create` rejects an empty
