...
/// file records are the same entries by construction. Two predicates
...
type private CoverageRead =
    ...
    /// `RunScope` rather than inventing one. The CLI's exit code
                        ...
                        let parsedCoverage =
                            ...
                            | _ ->
                                ...
                                if projectCount = 0 then
                                    ...
                                    // The MIXED case, and the one this
...
/// `WaitForComplete` RPC: the shared owner snapshot must contain no live work and
/// a current-model receipt earned from actual outcomes. `isSettled` observes that
/// RPC's completion. The status reads here are for RENDERING ONLY and never
/// decide the verdict.
...
/// instant it settled.
///
/// Deliberately NOT computed inside `publishVerdict`. Everything
...
let private publishVerdictWithReason
    // The invocation this verdict belongs to: the id it is stamped
    ...
    // The check-vs-confirm sample this run can offer: an EXECUTED reading
    ...
    // RETURNS the exit code it wrote, so the caller cannot compute a
    ...
    : int =
    try
        // EVERY batch this check has evidence from, not just the one the
        ...
        // `confirm` escalates a filtered scope to a forced full run and
        ...
        // Classified against `verdictOutcome` — the outcome this verdict
        ...
        // The declared gaps in this run's scope, re-read from
        ...
        // Where the wall time went. Every interval is placed against the
...
let pollAndRenderForInvocation
    ...
    // Set once, at the escalation below, and read again on every publish
    ...
    try
        ...
        // The exit code is the one `publishVerdict` WROTE, not a second
