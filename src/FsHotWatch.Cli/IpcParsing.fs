...
/// The full-suite run an impact-filtered green is RELATIVE to — the one
...
type TestScope =
    ...
    /// the earlier filtered run's reading (`Verdict.scopeToRecord`), and a
...
/// Ask what `check`'s impact selection WOULD have executed in the run
...
type TestRunReport =
    {
        ...
        ///
        /// It is ONE run of the several a check provokes — the one the
        ...
        RunId: Guid option
        /// Every run the daemon has COMPLETED in this session, newest
        ...
        /// The runs THIS CHECK is accountable for: the session runs the
        ...
        SeedCount: int
        /// The full-suite baseline the daemon's greens are relative to.
...
let parseTestRunReport (json: string) : TestRunReport =
    try
        ...
        let noTestsReason () =
            ...
            // Absent from an older daemon — silence, never a claim.
        ...
        // Absent from a daemon that predates the field — silence, not
        ...
        // Fails CLOSED like the scope: a `baseline` object this build
...
/// Did the impact selection `check` WOULD have used reach a test this
/// run saw fail?
///
            ...
            | JsonValueKind.Object, (true, phases) when phases.ValueKind = JsonValueKind.Array ->
                ...
                |> fun phases ->
                        ...
                        | true, values when values.ValueKind = JsonValueKind.Array ->
                            [ for value in values.EnumerateArray() do
                                  if value.ValueKind = JsonValueKind.Object then
                                      match
                                          value.TryGetProperty("runId"),
                                          value.TryGetProperty("modelGeneration"),
                                          value.TryGetProperty("refusals")
                                      with
                                      | (true, run), (true, generation), (true, refusals) when
                                          generation.ValueKind = JsonValueKind.Number
                                          && refusals.ValueKind = JsonValueKind.Array
                                          ->
                                          let runId =
                    ...
                    DaemonEvidence.Served(
                        phases,
                        model |> Option.defaultValue FsHotWatch.ProjectModel.Observation.Unobserved,
                        receipts
                    )
