...
type Command =
    ...
    | [<CmdExample("", "--run-once");
        ...
        RunFlag list
    /// Run the full suite and CONFIRM THAT `check` TOLD THE TRUTH (the tracked issue,
    /// the tracked issue). Same checks as `check`, but the tests run UNFILTERED — and the
    ...
    ///
    /// It was called `gate`, and the name did real damage: `gate`
    ...
    ///
    /// Reporting that comparison is part 2; naming it correctly is what
    /// makes it obvious there is one to make.
    ///
    /// `--run-once` runs it WITHOUT a daemon — which is how CI must
    ...
    /// accounting and produced the next content-free green. The act of
...
/// The corrupted-pipe fault family. StreamJsonRpc reads a
...
///
/// This is how `fshw confirm` FORCES the run it demands. `set-scope
...
/// Binary staleness is the DaemonIdentity handshake's job:
...
type RunningDaemonAction =
    ...
    /// handshake). Restart it with THIS binary. (a new CLI
...
let private ensureDaemon
    ...
    if not (ipc.IsRunning pipeName) then
        ...
        // The identity handshake: compare the running daemon's
...
/// Stale pidfile hygiene: delete `.fshw/daemon.pid` when the
...
/// CLI's: the fault is NAMED — status output computed by the
...
/// hooks. See the section header above for the full contract.
///
...
let executeCommand
    ...
    | None ->
        ...
        // Forced restart for corrupted-pipe self-healing: stop
        ...
        | Start ->
            ...
            | None ->
                ...
                | Some lockStream ->
                    ...
                    // Record THIS binary's identity BEFORE the
        ...
        | Scan ->
            ...
            withIpc (fun () ->
                ...
                0)
        | Status pluginName ->
            // Say it in words: a status computed by a daemon
        ...
        | Init ->
            ...
            with :? IOException ->
                ...
                1
        // Both `check` arms bracket the run with the run-level hooks.
        ...
        | Confirm flags ->
            // the evidence may ALREADY have been earned.
            //
            ...
            | Verdict.PriorConfirmation.StillApplies v ->
                // DELIBERATELY UNWRAPPED by run-level hooks: this
            ...
            | Verdict.PriorConfirmation.MustEarn ->
                ...
                // only reason CI can invoke `confirm` at all.
                //
                // Bracketed with the run-level hooks: this arm does the
...
let classifyParse (parsed: Result<GlobalFlag list * Command, ParseError>) : ParseDispatch =
    ...
    | Error VersionRequested ->
        ...
        // name the source ref this binary was built from — the
