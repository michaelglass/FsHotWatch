...
/// with failing diagnostics; a run that VERIFIED NOTHING) has to hold
...
let private renderCompact
    ...
    let line =
        ...
        | StatusView.Completed _ ->
            ...
            // rule that a run which VERIFIED NOTHING is not a ✓. The
...
module private Agent =
    ...
    /// Intelli…`, a list cut off mid-name (AC2 "names every affected
...
module AgentHints =
    ...
    let private priorEvidenceLines (prior: Verdict.Verdict option) (v: Verdict.Verdict) : string list =
        ...
        | Some p when p.TreeHash = v.TreeHash && not (List.isEmpty p.Suites) ->
            ...
            @ triggerLines
            // "across N batches" when the check ran the tests more than
    ...
    let forVerdict (runExists: System.Guid -> bool) (prior: Verdict.Verdict option) (v: Verdict.Verdict) : string list =
        ...
        let zeroSelectionLines =
            ...
            | NoTestsRun(NoTestsReason.ChangesUncovered(symbols, total, unrunnable)) ->
                ...
                [ $"    ZERO     ⚠ NOTHING COVERS THIS CHANGE — %d{total} changed symbol(s) have no covering test."
                  ...
                  // The write-off, where the reader is looking: these symbols
        ...
        // The red that reddened a `confirm` with all four plugins `ok`
        ...
        let ledgerCauseLines =
            ...
            | causes ->
                ...
                // AC5. The mixed case: SOME causes are stale and some are
        ...
        // ORDER IS THE FIX (follow-up). Every one of these lines already
        ...
        // Where the wall time went, over and above the plugin lines:
        ...
        // FIRST, ahead of the cause header. A recall miss is a fact about
