                    ...
                    (async {
                        try
                            ...
                            let cmdResult =
                                ...
                                | ProcessOutcome.Failed(code, _) when Some code = notEvaluatedExitCode ->
                                    NotEvaluated output
                            ...
                            | NotEvaluated _, _ ->
                                ...
                                let reason =
                                    if String.IsNullOrWhiteSpace output then
                                        $"%s{nameStr}: evaluation declined with the configured not-evaluated exit code"
                                    else
                                        $"%s{nameStr}: not evaluated — %s{output.Trim()}"
                            ...
                            ctx.EmitCommandCompleted(
                                { Name = nameStr
                                  Outcome =
                                    ...
                                    | NotEvaluated _ -> FsHotWatch.Events.CommandNotEvaluated output
                                    | CommandFailed _
        ...
        [ $"%s{nameStr}-status",
          fun _ctx state _args ->
              async {
                  ...
                  | NotEvaluated output ->
                      return JsonSerializer.Serialize({| status = "not-evaluated"; reason = output |})
      ...
      CacheKey =
        ...
        let cacheKey (event: PluginEvent<unit>) : ContentHash option =
            ...
            | FileChanged _ ->
                ...
                let evaluationPolicy = notEvaluatedExitCode |> Option.map string |> Option.defaultValue "none"
                Some(ContentHash.create (salt + ":not-evaluated=" + evaluationPolicy))
            | _ -> None
