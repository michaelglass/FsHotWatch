...
/// `artifactGateReddens` decides whether a stale-artifact
...
let createWith
    ...
    /// repo would rebuild on every `check`: the inner-loop regression the
    ...
    /// the instrument: name a finding this mode is not acting on.
    let reportWithoutActing (s: StaleArtifact) =
        ...
        info "build" $"artifact-gate (report-only): would have reported %s{s.Project}: %s{detail}"
    ...
    /// `force-rebuild` uses. The worst case is one real build, whose
    ...
    ///     thing the tracked issue exists to stop, and it is the highest-frequency
    ...
    ///     the own acceptance forbids, reached from the opposite direction.
...
/// asserts the behaviour are unchanged — those tests are the
