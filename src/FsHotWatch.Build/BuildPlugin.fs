...
let create
    ...
    { Name = PluginName.create "build"
        ...
        fun ctx state event ->
            async {
                ...
                | Custom(BuildDone(outcome, entries, elapsed)) ->
                    // a build has now actually run, which is exactly
      ...
      Commands =
        [ // the tracked issue. Idempotent and cheap: it sets a flag, it does not
      ...
      CacheKey =
        ...
        let cacheKey (event: PluginEvent<BuildMsg>) : ContentHash option =
            // while `forceRebuild` is set, the LOOKUP must miss so a
