...
- build input cache keys now identify repository files relative
...
- Case 1: artifact freshness is now enforced in production. A stored
...
- a nominally successful build now resolves each `MSB3026`
...
- **a cache replay is now refused over a dependency copy the build still
...
- `unrepairedCopies` is the floor that makes the new refusal terminate — a
  ...
  again.
- the plugin now logs how many dependency copies it examined, once per
...
- **a cache replay is now actually refused over a MISSING build output
  — in the daemon, where it never was.** The replay gate shipped correct and inert: the
  daemon constructs this plugin with `artifactGateReddens = false`,
...
- a build that reports success and produces no output for a project
...
- no behaviour change here, but the artifact gate this plugin owns can
...
- fix: **the build inputs merkle hashes MSBuild's implicit imports** (case 2). `Directory.Build.props`, `Directory.Build.targets` and
...
- fix: re-verify artifacts at cache-REPLAY time, so a cache hit can never assert
  freshness it has not confirmed
...
- fix!: adapt to the core `RunClaim` / verdict-carrying terminals: the
...
- fix!: adapt to core `RunVerdict`: the build's `Completed` status
