    ...
    member _.EnqueueIntent(key: string, coalescingKey: string option, deliver: WorkId -> unit) : Task<unit> =
        let immediate, receipt =
            mutate (fun snapshot ->
                ...
                | None ->
                    { snapshot with
                        Phase = Map.add intent.Id (Event(Some intent.Receipt, None)) work |> phase },
                        ...
                        |> Map.fold
                            (fun (remaining, receipts) id kind ->
                                ...
                                | Committing(receipt, pending) ->
                                    let queuedReceipts =
                                        pending
                                        |> Option.map (snd >> queued >> List.map (fun intent -> intent.Receipt))
                                        |> Option.defaultValue []
    ...
    member _.FailRun(id: WorkId, failure: exn) =
        let pending =
            mutate (fun snapshot ->
                ...
                let work, delivered =
                    admitSuccessor key pending (entries snapshot.Phase |> Map.remove id)
