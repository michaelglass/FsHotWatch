...
type Store() =
    ...
    let mutate change =
        // Internal ownership transitions must observe their actual publication.
        // A timed-out wait cannot roll back a queued mutation and would discard
        // its receipt, successor delivery or transferred event identity.
        // External operation deadlines remain separate from this settlement.
    ...
    member _.Observe() : IDisposable =
        let id =
            mutate (fun snapshot ->
                ...
                { snapshot with
                    Observers = Set.add id snapshot.Observers },
                id)
    ...
    member _.BeginOperation(name: string) =
        mutate (fun snapshot ->
            ...
            { snapshot with
                ...
                HostFailures =
                    snapshot.HostFailures
                    |> Map.filter (fun failedId (failedName, _) ->
                        failedName <> name || Map.containsKey failedId snapshot.Operations) },
            id)
    ...
    member _.FailOperation(id: WorkId, failure: exn) =
        mutate (fun snapshot ->
            ...
            | Some name ->
                { snapshot with
                    HostFailures = Map.add id (name, failure) snapshot.HostFailures },
                ()
            | None -> snapshot, ())
    ...
    let admitSuccessor key pending work =
        ...
        | first :: rest ->
            let active =
                work
                |> Map.toList
                |> List.tryPick (fun (identity, kind) ->
                    match kind with
        ...
        let immediate, receipt =
            mutate (fun snapshot ->
                    ...
                    [ yield!
                      ...
                      yield!
                          ...
                          |> List.choose (fun (id, kind) ->
                              ...
                              | Committing(_, Some(candidate, pending, _)) when candidate = key ->
                                  Some(id, kind, pending)
                              | _ -> None) ]

                let replacement =
                    candidates
                    |> List.tryPick (fun ((_, _, pending) as candidate) ->
                        coalescingKey
                ...
                | None ->
                    { snapshot with
                        Phase =
                            Map.add intent.Id (Event(Some intent.Receipt, Some(key, Running, IntentFold))) work
                            |> phase },
                ...
                | Some(id, kind, pending) ->
                    let next =
                        (match existing with
                         ...
                         | Some prior ->
                             queued pending
                             |> List.map (fun item -> if item.Id = prior.Id then intent else item))
                        |> slot
                    ...
                    { snapshot with
                        Phase = Map.add id kind work |> phase },
                    (None, intent.Receipt.Task))
            ...
            mutate (fun snapshot ->
                let payload =
                    requireKind
                        id
                        (function
                        | Event(receipt, continuation) -> Some(receipt, continuation)
                        | _ -> None)
                        snapshot
    ...
    member _.PublishEventState(id: WorkId, state: 'State) =
        mutate (fun snapshot ->
            ...
            { snapshot with
                ...
                Phase =
                    entries snapshot.Phase
                    |> Map.add id (Committing(receipt, continuation))
                    |> phase },
            ())
                        ...
                        |> Map.fold
                            (fun (remaining, receipts) id kind ->
                                ...
                                | Committing(receipt, pending) ->
                                    let queuedReceipts =
                                        pending
                                        |> Option.map (fun (_, slot, _) ->
                                            queued slot |> List.map (fun intent -> intent.Receipt))
                                        |> Option.defaultValue []
    ...
    member _.FailRun(id: WorkId, failure: exn) =
        let pending =
            mutate (fun snapshot ->
                ...
                let work, delivered =
                    admitSuccessor key (queued pending) (entries snapshot.Phase |> Map.remove id)
