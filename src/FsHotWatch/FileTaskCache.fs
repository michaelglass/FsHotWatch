...
let private serializeTestResult (key: string) (result: TestResult) =
    ...
    | TestsNoMatch(output, elapsed) ->
        ...
        // written as. Before the tracked issue this round-tripped as "passed" with the
...
let private deserializeTestResult (obj: JsonObject) : string * TestResult =
    ...
    let result =
        ...
        // back-compat, and it must come BEFORE the plain "passed" read.
...
let private serializeCachedEvent (evt: CachedEvent) =
    ...
    | CachedTestRunCompleted completed ->
        ...
        // The verification token, not a bool. `RunVerification.token`
...
/// 3: `testRunCompleted` carries a `verification` TOKEN where
