...
/// returning `""` — a failure to DO the work, rendered
...
/// kill that returned, `Some ex` for one that threw.
///
...
/// stream EOF.
///
/// a wall clock is the RIGHT bound here and the only sound one —
...
let runProcess
    ...
    //
    // it used to be a `task {}` over `ReadAsync`, whose every
    ...
    try
        ...
        //The policy itself lives in `killTreeWith`, where every arm
