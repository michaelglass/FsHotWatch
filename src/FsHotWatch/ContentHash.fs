...
///
/// Deliberately NOT a valid hex digest: a consumer eyeballing a manifest can see
/// that a file was unreadable, rather than being handed a plausible-looking hash of
/// nothing.
[<Literal>]
let UnreadableSentinel = "<unreadable>"
...
/// large), or `UnreadableSentinel` if it cannot be read.
///
...
let ofFile (path: string) : string =
    ...
    with
    | :? IOException
    | :? UnauthorizedAccessException -> UnreadableSentinel

/// True when `hash` is a real digest rather than the "I could not read it" sentinel.
/// Callers that need to REFUSE (rather than merely record) an unreadable input ask
/// this instead of string-matching the sentinel themselves.
let isReadable (hash: string) : bool =
    not (String.Equals(hash, UnreadableSentinel, StringComparison.Ordinal))
