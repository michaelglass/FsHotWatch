...
type TestResult =
    ...
    ///That made "we ran nothing" a SUB-CASE of "we passed", so
...
module TestResult =
    ...
    let isPassed =
        ...
        // The tracked issue was. That question is `verificationOf` (→ `RunVerification`),
    ...
    /// in two packages the moment it was needed. Since the tracked issue the run-level
...
///It replaces a free-standing `RanFullSuite: bool` that every
...
type RunVerification =
    ...
    ///
    /// This case did not exist, and its absence is why two
...
type TestRunCompleted =
    {
        ...
        ///
        /// This was `RanFullSuite: bool`. A boolean had no
