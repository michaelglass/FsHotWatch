/// Always-on append-only log of every plugin cache lookup. Each lookup
/// appends one line:
///
...
/// thellma's, etc.) write to one place — point an awk aggregator at it to
/// compute per-plugin hit rate without trawling each daemon's verbose log.
///
...
/// Format a single lookup line. Pure: separated from IO so tests can verify
