...
/// hung an IPC caller forever and let `test-rerun` exit 0 having
...
/// status model. That capability is exactly how the tracked issue happened (the
...
type PluginHostServices =
    {
        ...
        /// path derives per-file entries' summaries from it:
...
/// The replay summary for a per-file cache entry, derived
...
let registerHandler (services: PluginHostServices) (handler: PluginHandler<'State, 'Msg>) : RegisteredPlugin =
    ...
    ///
    /// The ownership guard used to be a check-then-act: it read
    ...
    /// per-file cache entry) must build that summary at the same
    ...
    //
    // ONE counter on purpose. The previous shape —
    ...
    let runExclusive (key: string) (work: Async<'Msg>) : RunClaim =
        ...
        // `statusLock`, so no terminal can slip between "no run
        ...
        let claimedAt =
            lock statusLock (fun () ->
                ...
                if shouldStart then
                    ...
                    // coverage was registered (review, finding 4).
        ...
        | ValueSome startedAt ->
            ...
            Claimed
        | ValueNone ->
            // item 5: exclusion-slot contention. The run is NOT
    ...
    // events — `Post` is the only path to work.
        ...
        MailboxProcessor<Choice<PluginEvent<'Msg>, AsyncReplyChannel<'State>>>
            .Start(
                (fun inbox ->
                    ...
                    let tryReplayCache (event: PluginEvent<'Msg>) (cacheKeyOpt: ContentHash option) =
                        ...
                        | Some cache, Some cacheKey ->
                            ...
                            | Some result ->
                                ...
                                // what the replayed verdict may say
                                ...
                                // a cached TERMINAL status must never
                                ...
                                // nobody earned — the exact false-green, and
                    ...
                    ///
                    /// EXCEPT while an exclusive run is in flight: the same
    ...
    { Name = handler.Name
      ...
      // run had not yet produced. Run tokens are released in
