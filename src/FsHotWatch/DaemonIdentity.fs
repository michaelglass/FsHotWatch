...
/// the daemon/CLI binary-identity handshake.
///
/// A new CLI silently talking to a daemon left over from an OLD build is the
/// trap (a version label that doesn't match its contents)
...
/// share a version string and differ in content (literally), so
...
/// file, one value for "I could not read it", repo-wide. Two
