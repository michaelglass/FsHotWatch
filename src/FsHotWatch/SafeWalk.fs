...
///   * A subtree it could not see is REPORTED, never silently dropped.
...
/// The two directories whose contents a build PRODUCES. Anything under one of
/// them is an output of the build, never an input to it — the single fact both
/// `SourceExcludedDirs` (for a walk) and `isBuildOutput` (for one known path)
/// are stated in terms of, so a walk and a lookup cannot drift about it.
let BuildOutputDirs = set [ "bin"; "obj" ]
...
let SourceExcludedDirs = Set.union ToolingExcludedDirs BuildOutputDirs

/// Is `path` a build OUTPUT of the project rooted at `projectDir`, rather than a
/// source of it?
///
/// The question a walk answers by pruning (`SourceExcludedDirs`), asked instead
/// about one path that is already in hand — a compile item MSBuild handed us,
/// which may well live under the project's own `obj/`
/// (`obj/<cfg>/<tfm>/<Project>.AssemblyInfo.fs` is a compile item of every SDK
/// project, regenerated by every design-time evaluation).
///
/// Asked RELATIVE to the project directory, never over the absolute path. Match
/// segments of the absolute path and any repo that happens to live under a
/// directory named `bin` or `obj` classifies its every file as build output —
/// and the caller, asking "is anything newer than the DLL", would then find
/// nothing and answer FRESH. That is the silent-degradation shape, arrived at by
/// being *more* thorough. (`.workspaces/` in `ToolingExcludedDirs` is the live
/// instance of the same trap: this project's own jj workspaces are full
/// checkouts nested under that name.)
let isBuildOutput (projectDir: string) (path: string) : bool =
    Path.GetRelativePath(projectDir, path).Split([| Path.DirectorySeparatorChar; Path.AltDirectorySeparatorChar |])
    |> Array.exists BuildOutputDirs.Contains
