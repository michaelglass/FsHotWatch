...
- fix: a file whose last FCS check reported errors is stamped
  ...
  - The call site diffed against the stored rows for BOTH diffable arms, which made
    `EverySymbolIsNew` behave exactly like `DiffAgainstStored`: the
...
- `test-scope` declares `runIds` — every run this daemon session has
...
- fix: a changed file that obligates no project no longer enters the
...
- fix: a run that widens past the zero-affected skip names every debt
...
- `failuresOf` no longer attaches the whole captured project output to
...
- fix: resolve the installed xUnit runner major from
...
- fix: keep one atomic run receipt for scope, run ID, seeds,
...
- stale dependency-copy refusals now identify the usual producer—a
...
- fix: only a green run whose terminal receipt establishes the
...
- feat: preserve the producing test project and consumer-ratchet
...
- `check-reach` now reports an exact `conditionalFailureRecall` numerator and denominator
...
- prior red tests are quarantined into the next ordinary impact run;
...
- decoded-literal selection seeds captured before TestPrune's destructive
...
- `ArtifactFreshness`'s copy rule now goes through core's
...
- **The impact selection `confirm` widens past is retained, and a new `check-reach`
  command reports whether it would have reached a failure.** The
...
- **A test host that was KILLED is now reported as an ABORT, not as N failed tests**.
  Under CPU load the gate returned large numbers of 0ms "failures" —
...
- A cold `confirm` full-suite run now discharges symbols discovered by its own BootScan
  cohort without scheduling the same suite a second time. The debt is
...
- The `--no-build` freshness gate fails CLOSED on a directory it could not read.
  `ArtifactFreshness.Cache.FilesUnder` returns
...
- fix: **a stale-build-output refusal is now telling the CLI which of the two "waiting
  on build" causes it is** (QA rework). Every refusal the stale-artifact
...
- fix: **the project-structure hash sees every MSBuild implicit import, not just
  `Directory.Build.props`** (case 1, follow-up). `Directory.Build.targets`
...
- fix: **a project that matched ZERO tests no longer discharges a symbol's test debt**.
  The per-symbol green-commit folded `TestResult.isPassed` over the
...
- fix!: **stale build output is detected BEFORE any suite runs, and the provable case is
  repaired**. The freshness gate was right — it refuses to run
...
- refactor: **the freshness sidecar's verdict over an emptied index is now a decision,
  not a by-product.** `.fshw/test-prune/file-freshness.json` carries no
  ...
  real test debt as a zero-test green. Measured against a real recreate
...
- feat: **every test project's output is saved, streamed, for every run**.
  `.fshw/test-runs/<runId>/<Project>.output.log`, written as the
...
- fix: **a schema recreate could discharge real test debt as a green that ran nothing.**
  `ChangedSymbolsAllUncovered` buys a "nothing to verify" green that
...
- feat: **poisoned pending-verification seeds are now named in the log.**
  A symbol leaves the needs-testing queue only when every runnable
  ...
  mis-qualified symbol ('s `name`/`kind`, which alone selected ~2,837
...
- fix: **`run-tests` says which project you asked for and which exist when nothing
  matches.** The reply was a bare `no matching test projects`, which is
...
- fix: **a run in which every project matched zero tests is no longer a green.**
  Two aggregators were blind to it in the same way. `recordRunOutcome`
...
- fix!: **a filtered re-run that PASSES retires the red it re-ran**.
...
- chore(deps): TestPrune.Core 6.1.1 — bounded post-exit output drain in
  `runProcessWith` (grandchild-held pipe can't wedge the drain).
...
- feat!: **`fshw gate` is now `fshw confirm`.** **Migration: `fshw gate`
...
- fix!: **an unreadable needs-testing ledger is not an empty one.**
...
- fix!: **a run may clear only what it COVERED.** A full run failed
  ...
  re-ran and never passed, yet `check` went green. Same disease:
...
- fix: an unanalysable-file warning no longer disappears at the first
...
- fix!: **the freshness gate judged a test project against the whole repo — so it
  ...
  caught.** `apphostStale` compared the test project's DLL against
...
- feat!: **CTRF reports are RETAINED, and the dead `.log` format is deleted.**
...
- fix!: **a force-run refused the slot is QUEUED, never declined.**
...
- fix!: **a force-run the daemon cannot see is a gate that cannot gate.**
  The `run-tests` IPC command executed the suite directly on the
...
- fix!: **a file whose symbol analysis failed was silently dropped from the impact
  ...
  *informational* parse diagnostic in the first place.
...
- fix: a cycle whose changed/queued symbols **all have no covering test** now
  ...
  passed", differing only in that zero ran. (QA finding: the
...
- fix: a test child that **never becomes a live process** no longer wedges the
  ...
  indefinitely. No fault was raised, so neither the faulted-run
  Failed path nor the beforeRun fix applied, and `check`'s
  ...
  turn into the SAME `Aborted` lifecycle a `beforeRun` throw does (seam) → `PluginStatus.Failed`, so `check` reads non-green with a legible
  ...
  no-output test failure). (QA finding: the launch gap)
...
- feat: the manual `run-tests` command's slot-wait budget (how long it waits for
  ...
  `waitSec` falls back to the default. (`parseRunTestsWaitMs` /
...
- fix: a failing `beforeRun` preflight in the manual `run-tests` command now
  ...
  constructions are unified in one helper so they can't drift.
...
- fix: a transient DB error while resolving covering projects (`QueryAffectedTests` per-symbol lookups) now yields the honest re-runnable `Aborted` lifecycle instead of escaping as a raw framework fault that stranded the run before any test process launched.
