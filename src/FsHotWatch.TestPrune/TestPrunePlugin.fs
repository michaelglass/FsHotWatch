...
type TestPruneState =
    {
        ...
        /// plugin's slice. It used to be a bare `Set<string>` and the
...
///
/// It is deliberately not an independent derivation. Until the tracked issue the gate's
...
module internal OutstandingFailure =
    ...
    /// red would wedge the gate forever — the stuck-red, rebuilt). Empty
...
let private executeTests
    ...
    async {
        ...
        // per-test flakiness records, COLLECTED here and written ONCE
                ...
                async {
                    ...
                    for config in groupConfigs do
                        ...
                        | false, None ->
                            ...
                            // that format is GONE. It was written only
                            ...
                            // the run.
                            //
                            // The report is RETAINED. It used to be
...
let internal cacheKeyFor
    ...
    // `Some "full"` while the merge gate has asked for the whole
    ...
    | Custom(TestsFinished(_, completed, _)) ->
        // 2026-06-07: a FAILED test outcome must never be served from
...
let create
    ...
    let runTestsWithImpact
        ...
        async {
            ...
            //
            //  * merge-gate scope: run EVERY project, unfiltered.
        ...
        | Some allConfigs when not allConfigs.IsEmpty ->
            ...
            @ [ "set-scope",
                fun (_ctx: CommandCtx<TestPruneMsg>) (_state: TestPruneState) (args: string array) ->
                    async {
                        // `fshw gate` calls this BEFORE triggering its
    ...
    { Name = PluginName.create "test-prune"
            ...
            async {
                ...
                | PluginEvent.FileChecked result ->
                    ...
                    try
                        ...
                        | Ok analysisResult ->
                            ...
                            return newState
                        | Error msg ->
                            // This branch used to `return state` — the file
                ...
                | PluginEvent.BatchChecked _ ->
                    ...
                    | Ok flushedState ->
                        ...
                        //
                        // the skip asks whether anything is owed, so it
      ...
      CacheKey =
        ...
        let cacheKey (event: PluginEvent<TestPruneMsg>) : ContentHash option =
            ...
            // a merge gate must never REPLAY an impact-filtered run's
