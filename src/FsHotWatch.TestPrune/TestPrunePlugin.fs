...
//
// The disease (arrived at by another road): "no
...
/// the same defect as a red erased by a run that never executed it.
...
/// the "this must deny the check a green verdict" quietly stopped holding.
...
/// never a superseded red (the "Issue 2" accumulation), and never a
/// laundered one.
...
///
/// `Selection` is the run's SCOPE: what it was launched against,
...
type TestPruneMsg =
    ...
    /// under the `RunExclusive "tests"` slot. The command must
...
/// green. Both the impact path (`runTestsWithImpact`) and the
...
///
/// Now a one-line delegation to `TestResult.isNoMatch`: since the tracked issue the fact
...
/// type.
///
/// The derivation itself moved to core in the tracked issue (`RunVerification.ofResults`),
    ...
    let projects =
        ...
        |> List.map (fun (name, result) ->
            let (status, output) =
                ...
                // wire string is UNCHANGED — only the way it is
...
module RunCoverage =
    ...
    ///     Without that the request claimed nothing, so
...
///
/// It is deliberately not an independent derivation. Until the tracked issue the caller's
...
///     executed;
///   * every unanalysable-file warning. The clear wiped those too, so
...
let private executeTests
    ...
    async {
        ...
        // Launch-liveness deadline (QA finding). Between a test
            ...
            |> List.map (fun (_, groupConfigs) ->
                async {
                    ...
                    for config in groupConfigs do
                        ...
                        // FRESHNESS GATE (mirrors BuildPlugin.verifyArtifactsFresh,
                        ...
                        | false, None ->
                            ...
                            // ONE DIRECTORY PER RUN. The reports of a run
                            ...
                            // THE RUN LOG. Opened for EVERY project, on
...
/// away (the comment defending it claimed "cacheKey runs once per
...
let internal cacheKeyFor
    ...
    // `Some "full"` while the caller has asked for the whole
    ...
    // Has a run in THIS PROCESS produced test evidence — i.e. is there
    ...
    //
    // salt bumped v1→v2 so any entry written by the prior code (which
    ...
    | BuildCompleted BuildSucceeded ->
        // 99. A cache HIT makes the framework replay the cached
        ...
        //
        // An outstanding failure refuses participation for the same
        ...
        // filtered run.
        //
        // And a process with NO test evidence of its own refuses for the
    ...
    | Custom(TestsFinished(_, completed, _)) ->
        ...
        // The tracked issue adds the third condition: a run that passed everything IT ran
        ...
        //
        // this arm is deliberately NOT gated on `sessionHasTestEvidence`.
...
let create
    ...
    /// full suite until a green, so a format change would hand every
    ...
    /// The reds no covering run has passed since, mirrored out of
    ...
    /// ('s stale-red-on-a-green-tree, in reverse).
    ...
    let flushAndQueryAffected (state: TestPruneState) =
        ...
        //
        // `not db.WasRecreated` is the third conjunct, and it is
    ...
    let runTestsWithImpact
        ...
        async {
            ...
            // advance the poisoned-seed counters HERE, not in
            ...
            try
                ...
                //
                //  (2) Nothing-to-verify (QA). This cycle HAD
                ...
                //      "empty queue" must mean PROVABLY empty, so this
                ...
                then
                    ...
                    let completed: TestRunCompleted =
                        { RunId = runId
                          ...
                          // two lies. This one is `Normal`, so it
            ...
            @ [ "set-scope",
                ...
                fun (ctx: CommandCtx<TestPruneMsg>) (state: TestPruneState) (args: string array) ->
                    async {
                        ...
                        // ran" verb. The run NEVER executes here:
                        ...
                        // the run didn't produce (: bound every seam).
                        try
                            let argStr = if args.Length > 0 then args.[0].Trim() else "{}"
        ...
        fun ctx state event ->
            async {
                ...
                | PluginEvent.FileChecked result ->
                    ...
                    // the ONE treatment for a file whose symbol
                    ...
                    try
                        ...
                        | Ok analysisResult ->
                            ...
                            // no per-plugin idle guard — the framework's
                            // ReportStatus funnel drops any terminal stamped while an
                    ...
                    with ex ->
                        // F10 ∩ 113: a fault ANYWHERE in this handler
                ...
                | Custom(TestsFinished(started, completed, launch)) ->
                    ...
                    // a run may clear ONLY what it COVERED.
                    //
                    // Issue 2 established that the ledger must be
                    ...
                    // moment such evidence exists (no stuck-red:).
                    //
                    // the launch selection cannot express the reach of a
                    ...
                    //    impact-FILTERED. Until the tracked issue this read `completed.RanFullSuite`,
                    ...
                    let recordRunOutcome (results: TestResults) =
                        ...
                        // §3c. Projects RAN, and every one of them
                        ...
                        | None ->
                            ...
                            // language the tracked issue removed from the verdict, still
                ...
                | Custom(RunTestsRequested(configs, filter, reply)) ->
                    // the `run-tests` force-run, launched from the
