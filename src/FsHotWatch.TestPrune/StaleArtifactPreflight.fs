...
/// like progress (hit live three times in one day). The comparison is
...
/// file's heal. A tripped breaker on a clean tree changes nothing,
...
/// How far back the breaker looks. Michael's starting point (approval):
...
///
/// it no longer says `dotnet build` either, and that is wrong in two
    ...
    let rec round (remaining: int) (ledger: HealRecord list) (healed: string list) : Outcome =
        ...
        | detected ->
            ...
            for (project, _, (origin, copy)) in repairable do
                Logging.info
                    "test-prune"
                    $"artifact-copy-differs: %s{project} — origin %s{origin} differs from copy %s{copy}"
            ...
            // These are the refusals this round has ALREADY earned —
