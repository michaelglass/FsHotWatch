...
///   pass predicate.
///
///   REVISED. This rule used to open by claiming it was "the one
...
module Detect =
    ...
    /// DELETED one and is kept here so that re-introducing it — the
    ...
    ///
    /// REVISED. Two legs of the original argument have since been
...
[<CliAnalyzer("RunVerdictFoldAnalyzer",
              "Forbids folding a per-project pass predicate over a whole run's results as the run-level verdict — forall is vacuously true for a run that selected no project")>]
