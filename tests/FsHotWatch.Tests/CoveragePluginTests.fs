...
///
/// This carried `Results = Map.empty` — a shape the plugin
...
/// degenerate lifecycles say `false`, but a replayed cache
...
let ``regression: impact-filtered run with stale baseline does not false-red on un-run file`` () =
    withTempDir "coverage" (fun dir ->
        ...
        //
        // This drove `Results = Map.empty`. That is not what a
...
let ``a run that executed NOTHING reaches no coverage verdict — its full-suite claim is vacuous`` () =
    // The gate is `if ranFullSuite then Failed else NotGatedFiltered`,
...
// ---------------------------------------------------------------------------
// The coverage check is VISIBLE while it runs (review, finding 4)
//
