...
let ``DiagnosticSeverity: every severity round-trips through its wire name, and Deferred ranks between Info and Warning``
    ...
    //
    // ENUMERATED BY REFLECTION, not by hand. This list was written out
    ...
    // `HostAborted` sits exactly where `Deferred` does, and for the same
    ...
    // Invalid evidence has the same non-failing rank while retaining
...
/// `DiagnosticCounts` is the projection the status renderer and the
