...
let withEveryPoolThreadBusy (body: unit -> 'T) : 'T =
    ...
    // and more than its minimum, up to which it adds a thread for each without delay.
    let minimum, _ = ThreadPool.GetMinThreads()

    let held =
        [ for _ in 1 .. max ThreadPool.ThreadCount minimum + Environment.ProcessorCount * 4 ->
              Tasks.Task.Run(fun () -> gate.Wait()) ]
...
/// timestamp by construction, only the measured duration — the replay
...
// ---------------------------------------------------------------------------
// cold-vs-cached ledger parity, against REAL plugin handlers
// ---------------------------------------------------------------------------
//
...
let outOfBatchFile = "/tmp/fshw-out-of-batch/Earlier.fs"

let outOfBatchEntry: FsHotWatch.ErrorLedger.ErrorEntry =
    FsHotWatch.ErrorLedger.ErrorEntry.error "out-of-batch sentinel"
