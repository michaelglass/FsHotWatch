...
[<Fact(Timeout = 30000)>]
let ``AUTOMATION-259 full-suite completion serves the narrower retained impact selection`` () =
...
[<Fact(Timeout = 30000)>]
let ``AUTOMATION-259 impact completion reports absent comparison instead of inventing a selection`` () =
...
[<Fact>]
let ``AUTOMATION-259 class-filter refusal preserves readable word spacing`` () =
    let selection =
