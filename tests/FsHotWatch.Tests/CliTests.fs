...
/// The help text is what a reader has instead of the repo's own rules:
...
// --- case 2: a persistent FSEvents refusal fails the daemon closed ---
...
// --- decideRunningDaemonAction tests (identity handshake) ---
...
let ``RerunFilter.render omits Project (it selects projects, it is not an xUnit filter)`` () =
    // `--project` picks WHICH test projects the daemon invokes and travels in
...
let ``RerunFilter.projects collects the named projects, de-duplicated`` () =
    ...
    // historical behaviour, which stays the default.
...
// --- the daemon self-heal, end to end through executeCommand ---
//
...
// --- a refusal must name WHAT it searched for and WHERE ---
//
...
[<Trait("OutcomeCompleteness", "MixedOutcome")>]
...
// ---------------------------------------------------------------------------
// a failing beforeRun step must SAY WHAT FAILED
// ---------------------------------------------------------------------------
//
...
module ``beforeRun failure reporting`` =
    ...
    /// Hook timings are filed under the run they belong to, so a later
...
let ``loaded configuration identity retains the parsed snapshot across later file changes`` exists =
    withTempDir "daemon-config-snapshot" (fun root ->
        ...
        Assert.NotEqual(computeConfigHashWith defaultFileOps root, identity))
