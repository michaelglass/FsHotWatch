...
let ``run-once publishes a verdict file that agrees with its exit code`` () =
    // Before the tracked issue the run-once path wrote NO verdict at all — so `fshw
...
let ``readTestRun on a host with NO test-scope command is ScopeUnknown, not full-suite`` () =
    // in miniature: the command `confirm` asks for did not exist, the host
...
// ---------------------------------------------------------------------------
// A FAULTED READING IS NOT A GOOD READING (the principle, applied to
// the TEST SCOPE).
//
...
let ``requestFullSuiteScope sends set-scope full to the host`` () =
    // The regression, pinned: `confirm` must address the COMMAND
