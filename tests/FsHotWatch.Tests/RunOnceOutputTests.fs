...
// ---------------------------------------------------------------------------
// `gate --run-once`: the merge verdict WITHOUT a daemon.
//
...
let ``readTestRun on a host with NO test-scope command is ScopeUnknown, not full-suite`` () =
    // in miniature: the command the gate asks for did not exist, the host
...
let ``requestFullSuiteScope sends set-scope full to the host`` () =
    // The regression, pinned: the gate must address the COMMAND
