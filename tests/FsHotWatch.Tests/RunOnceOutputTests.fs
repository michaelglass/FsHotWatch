...
// --- total discovery failure — files found, none loaded ---
//
...
// ---------------------------------------------------------------------------
// `confirm --run-once`: the merge verdict WITHOUT a daemon.
//
...
let ``run-once publishes a verdict file that agrees with its exit code`` () =
    ...
    // reported "no verdict on disk". File and exit code are two renderings
...
let ``readTestRun on a host with NO test-scope command is ScopeUnknown, not full-suite`` () =
    ...
    // as "I could not establish what ran", never round up to full-suite.
...
let ``requestFullSuiteScope sends set-scope full to the host`` () =
    // `confirm` must address the COMMAND (`set-scope`), not the plugin
...
// ---------------------------------------------------------------------------
// the SECOND transport, over the same defect.
//
