...
let ``nested solution reconciliation refuses omitted real test projects`` () =
    withTempDir "scope-nested-reconcile" (fun root ->
        stageRepo root "Repo.slnx" [ "tests/P1", testProjectXml; "tests/P2", testProjectXml ]
        let nested = Path.Combine(root, "solutions", "Chosen.slnx")
        Directory.CreateDirectory(Path.GetDirectoryName nested) |> ignore
        let original = Path.Combine(root, "Repo.slnx")
        File.WriteAllText(nested, File.ReadAllText(original).Replace("Path=\"tests/", "Path=\"../tests/"))
        File.Delete original
        let configured = [ gated "P1" "run --project tests/P1 --" ]
        let findings = reconcile root (Some "solutions/Chosen.slnx") configured []
        Assert.Contains(UndeclaredTestProject "tests/P2", findings)
        ...
        let governed =
            reconcile
                ...
                (Some "solutions/Chosen.slnx")
                configured
                [ excluded "tests/P2" "separate integration gate" ]
