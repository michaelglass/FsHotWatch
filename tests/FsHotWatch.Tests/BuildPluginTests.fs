...
/// A handler's cache-key function, applied to its initial state.
let private keyOf (handler: PluginHandler<'State, 'Msg>) = handler.CacheKey.Value handler.Init
...
// ---------------------------------------------------------------------------
// CASE 2 — a cached build may not hide a compile item it never saw.
//
...
[<Fact(Timeout = 15000)>]
let ``case 2: a compile item added to Directory.Build.props moves the build merkle`` () =
    ...
    withTempDir "case2-props" (fun root ->
...
[<Fact(Timeout = 15000)>]
let ``case 2: Directory.Build.targets and Directory.Packages.props move it too`` () =
    ...
    // lives.
    withTempDir "case2-imports" (fun root ->
        let projDir = System.IO.Path.Combine(root, "src")
        System.IO.Directory.CreateDirectory projDir |> ignore
        let proj = System.IO.Path.Combine(projDir, "Thing.fsproj")
        System.IO.File.WriteAllText(proj, "<Project />")
...
[<Fact(Timeout = 15000)>]
let ``case 2: an unrelated file beside the implicit imports does NOT move it`` () =
    ...
    withTempDir "case2-floor" (fun root ->
        let projDir = System.IO.Path.Combine(root, "src")
        System.IO.Directory.CreateDirectory projDir |> ignore
        let proj = System.IO.Path.Combine(projDir, "Thing.fsproj")
        System.IO.File.WriteAllText(proj, "<Project />")
...
/// MSBuild reported — never `RegisterFromFsproj`'s XML parse (every test
...
[<Fact(Timeout = 15000)>]
let ``production build cannot mint success when its canonical DLL is missing`` () =
    let getBuild =
        runVerifyHarness
            "build-verify-missing"
            (TimeSpan.FromMinutes(-5.0))
            (TimeSpan.FromMinutes(-5.0))
            TimeSpan.Zero
            false
...
// ---------------------------------------------------------------------------
// `force-rebuild`: a cache hit must not assert freshness it never
// verified.
//
...
/// from what MSBuild reported. While these fixtures used
...
// ---------------------------------------------------------------------------
// Case 1 production enforcement. `createWith false` is the exact legacy
// wiring the daemon used during the observation window. The parameter is
// retained for source compatibility but cannot disable the invariant now. Each absence
// assertion is paired with a positive control on the same detector.
// ---------------------------------------------------------------------------

/// The legacy production call shape, retained as a regression boundary.
let private legacyProductionPlugin graph =
    BuildPlugin.createWith false "true" "" [] graph [] None [] None

[<Fact(Timeout = 15000)>]
let ``legacy production wiring refuses a missing build output`` () =
    ...
    withOneProjectGraph "replay-reportonly-missing" (fun (graph, srcPath, dllPath) ->
        let cacheKeyFn = keyOf (legacyProductionPlugin graph)
        let fileEvt = FileChanged(SourceChanged [ srcPath ])
        ...
        // everything and turned the build cache off.
        let withDll = cacheKeyFn fileEvt
        test <@ withDll.IsSome @>
...
[<Fact(Timeout = 15000)>]
let ``production wiring refuses an mtime-stale cache replay`` () =
    // Case 1 promotes the corrected detector. The old
    // report-only window served the stored success here, leaving TestPrune waiting on
    // a build that the cache promised had already happened.
    withOneProjectGraph "replay-reportonly-mtime" (fun (graph, srcPath, dllPath) ->
        let fileEvt = FileChanged(SourceChanged [ srcPath ])
        let dllTime = System.IO.File.GetLastWriteTimeUtc dllPath
        System.IO.File.SetLastWriteTimeUtc(srcPath, dllTime.AddMinutes 1.0)

        let production = keyOf (legacyProductionPlugin graph) fileEvt
        test <@ production.IsNone @>)

[<Fact(Timeout = 30000)>]
let ``a build that leaves an output missing cannot make cached success replayable`` () =
    // A command that omits a graph project is a configuration failure, not evidence
    // that an absent output is fresh. The old one-build exemption served the stored
    // success on the second lookup even though the DLL remained absent.
    withOneProjectGraph "replay-unproduced" (fun (graph, srcPath, dllPath) ->
        let ranMarker =
            System.IO.Path.Combine(System.IO.Path.GetDirectoryName dllPath, "build-ran")
        ...
        let handler =
            BuildPlugin.createWith false "touch" ranMarker [] graph [] None [] None

        host.RegisterHandler(handler)

        System.IO.File.Delete dllPath

        // The subprocess exits zero but leaves the attributed output missing.
        host.EmitFileChanged(SourceChanged [ srcPath ])
        waitForTerminalStatus host "build" 20000
        let firstRun = System.IO.File.GetLastWriteTimeUtc ranMarker

        System.Threading.Thread.Sleep 20
        host.EmitFileChanged(SourceChanged [ srcPath ])

        let rebuilt =
            waitUntilTrue (fun () -> System.IO.File.GetLastWriteTimeUtc ranMarker > firstRun) 15000

        test <@ rebuilt @>
        test <@ not ((terminalSummary host).Contains "(cached)") @>
        test <@ not (System.IO.File.Exists dllPath) @>)

// ---------------------------------------------------------------------------
// QA rework — the gate has to cover every event that READS the
...
//     an ungated key. For exactly those repos, both `force-rebuild`
...
// ---------------------------------------------------------------------------
// BUILD's own cold-vs-cached ledger parity
// ---------------------------------------------------------------------------

[<Fact(Timeout = 30000)>]
let ``a cached build replay leaves an out-of-batch finding standing`` () =
    ...
    withOneProjectGraph "build-parity" (fun (graph, srcPath, _dllPath) ->
        let cache =
            FsHotWatch.TaskCache.InMemoryTaskCache() :> FsHotWatch.TaskCache.ITaskCache
...
// ---------------------------------------------------------------------------
// QA rework — the COPY, which is what the wedge is actually made of.
//
...
[<Fact(Timeout = 15000)>]
let ``legacy production wiring serves a settled dependency copy`` () =
...
[<Fact(Timeout = 30000)>]
let ``a build that leaves a dependency copy divergent cannot make cached success replayable`` () =
    // A byte-divergent copy is attributable stale code. A successful subprocess that
    // leaves it divergent does not license replaying the prior success on the next
    // lookup; the build must run again until the copy is actually repaired.
    withProducerConsumerGraph "replay-copy-floor" false (fun (graph, srcPath, copy) ->
        let ranMarker =
            System.IO.Path.Combine(System.IO.Path.GetDirectoryName copy, "build-ran")

        let cache =
            FsHotWatch.TaskCache.InMemoryTaskCache() :> FsHotWatch.TaskCache.ITaskCache

        let host = PluginHost(Unchecked.defaultof<_>, "/tmp", taskCache = cache)
        host.RegisterHandler(BuildPlugin.createWith false "touch" ranMarker [] graph [] None [] None)
