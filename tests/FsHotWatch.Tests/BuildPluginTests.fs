...
let private runVerifyHarness
    ...
    withTempDir label (fun tmpDir ->
        ...
        // Production used to pass `false` here while AUTOMATION-368 observed the
...
// ---------------------------------------------------------------------------
// AUTOMATION-358 Case 1 production enforcement. `createWith false` is the exact legacy
// wiring the daemon used during the AUTOMATION-368 observation window. The parameter is
...
let ``production wiring refuses an mtime-stale cache replay`` () =
    // AUTOMATION-358 Case 1 promotes the corrected AUTOMATION-368 detector. The old
...
[<Fact(Timeout = 15000)>]
[<Trait("A104Evidence", "BuildCacheSnapshot")>]
...
[<Fact(Timeout = 15000)>]
[<Trait("A104Evidence", "ForceRebuildSnapshot")>]
