...
// ---------------------------------------------------------------------------
// production reachability — every test above builds the plugin with
// `create`, i.e. `artifactGateReddens = true`. NO LIVE DAEMON DOES: `DaemonConfig`
// passes `false` so the tracked issue can hold the mtime reading back until it has been
...
[<Fact(Timeout = 15000)>]
let ``report-only: an mtime-stale output still replays, as intends`` () =
