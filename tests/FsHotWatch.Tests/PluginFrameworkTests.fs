...
let ``RunExclusive forces a terminal Failed status when work raises (no strand)`` () =
    // Regression for the tracked issue (fresh-workspace daemon wedge). A faulted
...
let ``a handler throw while an exclusive run is in flight does not stomp a terminal status over it`` () =
    ...
    // Running IS the manufactured-terminal lie. The crash is
...
// `elapsed:`) that exist to make impossible.
//
...
// ---------------------------------------------------------------------------
// the derived per-file replay summary feeds `RunVerdict.create`,
