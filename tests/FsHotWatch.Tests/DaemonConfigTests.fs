...
// --- Global timeoutSec default (item 1) ---
...
let ``parseConfig keeps the run-level beforeRun separate from tests.beforeRun`` () =
    ...
    // a string in the config is now a ONE-STEP chain.
...
[<Fact(Timeout = 15000)>]
let ``coverage can collect for impact without joining the ratchet`` () =
    let json =
        """{
        "tests": {
            "projects": [{
...
// --- loadConfig: a verdictInputs declaration is never half-understood ---
...
let ``registered test owner honors the actual declared project identity`` () =
    // The exclusion names a DIRECTORY; the symbol index keys tests by the project FILE's
    // name. Registration must hand the plugin the discovered file name, or the declared
    // project keeps owing debt it was declared out of.
    withTempDir "cfg-test-scope" (fun tmpDir ->
        ...
        try
            ...
            match daemon.Host.GetStatus "test-prune" with
            | Some(FsHotWatch.Events.PluginStatus.Failed(message, _, _)) ->
                Assert.Fail($"a declared, reasoned exclusion must not leave the plugin red: %s{message}")
            | _ -> ()
        finally
