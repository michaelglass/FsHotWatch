...
let ``parseConfig keeps the run-level beforeRun separate from tests.beforeRun`` () =
    // The top-level `beforeRun` (run-level) and `tests.beforeRun`
...
// `"cache": "file"` / `"jj"` selected an on-disk FCS check cache
...
let ``stripConfig preserves the run-level hooks (run-once must honor them)`` () =
    ...
    // bracket must survive the run-once strip untouched.
    let withHooks =
        { defaults with
...
// ---------------------------------------------------------------------------
// makeShellHookWithResult — the `beforeRun` hook must be BOUNDED
// ---------------------------------------------------------------------------
//
