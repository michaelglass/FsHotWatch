...
[<Fact(Timeout = 30000)>]
let ``a zero-symbol FileChecked forces its runtime-covered project in full`` () =
...
[<Fact(Timeout = 30000)>]
let ``FileChecked exception while tests running surfaces in the ledger without stomping the run's status (F10)`` () =
...
[<Fact(Timeout = 60000)>]
let ``seeded DB + absent sidecar: string-literal change re-selects the asserting test (instance 1)`` () =
...
[<Fact(Timeout = 60000)>]
let ``seeded DB + absent sidecar: DU-list length change re-selects the count-asserting test (instance 2)`` () =
...
// --- parseRunTestsWaitMs (run-tests slot-wait budget) ---
...
[<Fact>]
let ``collected coverage retains test-project identity and ratchet intent`` () =
...
[<Fact>]
let ``runtime selector widens missing and stale configured projects with typed reasons`` () =
...
[<Fact>]
let ``runtime widening is diagnosed when the AST change has zero symbols`` () =
    let observedAt =
...
[<Fact>]
let ``runtime file obligations survive restart without cross-producting projects`` () =
...
[<Fact>]
let ``a repeated pair arriving mid-run survives the older green generation`` () =
    let launched =
...
[<Fact>]
let ``restart drops obligations for projects no longer configured for collection`` () =
    let obligations =
        mergeRuntimeCoverageObligations
            Map.empty
...
[<Fact>]
let ``interrupted obligation persistence restarts as unknown debt`` () =
...
[<Fact>]
let ``a ledger write failure leaves recovery armed before restart`` () =
...
[<Fact>]
let ``a recovery marker failure does not accept an undurable obligation transition`` () =
    let before =
...
[<Fact>]
let ``a changed file that obligates no project records no runtime debt`` () =
...
[<Fact>]
let ``an untraced file in the same cycle as a traced one owes only the traced one`` () =
...
[<Fact>]
let ``a later cycle finding nothing to add leaves a real obligation standing`` () =
...
[<Fact>]
let ``selectByRuntimeCoverage over an untraced file leaves the ledger empty`` () =
...
[<Fact>]
let ``no runtime obligation ledger transition may name zero projects`` () =
...
[<Fact>]
let ``collect-only coverage stays project-attributed and out of ratchet output`` () =
...
[<Fact>]
let ``disabling a formerly ratcheted project removes its persisted contribution`` () =
...
[<Fact>]
let ``first project-attributed ingest invalidates a legacy projectless ratchet`` () =
...
[<Fact(Timeout = 30000)>]
let ``project-filtered run keeps unrelated configured ratchet projects eligible`` () =
...
[<Fact>]
let ``coverage receipt removes stale artifact and accepts only a newly written successful result`` () =
...
[<Fact>]
let ``an unreadable old artifact becoming readable is not a new-run receipt`` () =
    let launch =
...
[<Fact>]
let ``failed or filtered launches cannot replace a complete runtime baseline`` () =
...
[<Fact>]
let ``successful full run without a fresh receipt revokes its baseline and denies green`` () =
...
[<Fact>]
let ``successful filtered run without a receipt preserves its full baseline`` () =
...
[<Fact>]
let ``successful full run diagnoses unreadable and unchanged stale receipts`` () =
    let prior =
        CoverageArtifactState.Fingerprinted
            { Length = 42L
              LastWriteUtc = DateTime.UnixEpoch
...
[<Fact>]
let ``malformed full receipt invalidates prior runtime evidence and denies green`` () =
...
[<Fact(Timeout = 15000)>]
let ``TestPrune CacheKey is None for a failing TestsFinished, Some for an all-pass`` () =
...
[<Fact(Timeout = 15000)>]
let ``a green partial receipt cannot write a whole-tree cache entry`` () =
    let started: TestRunStarted =
        { RunId = Guid.NewGuid()
          StartedAt = DateTime.UtcNow }
...
[<Fact(Timeout = 15000)>]
let ``an unfiltered project subset receives partial verification`` () =
...
[<Fact(Timeout = 20000)>]
let ``a project-scoped rerun cannot satisfy the next whole-suite check`` () =
    withTempDir "tp-partial-cache" (fun tmpDir ->
        let cache = FsHotWatch.TaskCache.InMemoryTaskCache()
        let cacheIface = cache :> FsHotWatch.TaskCache.ITaskCache
        let host = PluginHost(Unchecked.defaultof<_>, tmpDir, taskCache = cacheIface)
...
[<Fact(Timeout = 20000)>]
let ``a failed test run is not cached, so a later run on the same key is a miss and reports green`` () =
    ...
    withTempDir "tp-fail-not-cached" (fun tmpDir ->
        ...
        // until this process has test evidence, and the question here is
...
//
// "Never diff against them" was always right. "Therefore contribute
...
"""
        ...
        // This read `= "[]"` before the fix, and that empty list IS the
...
// direction 1 — the false positive, verbatim: a build tool was edited and
...
/// direction A — the dangerous one, and the one no other case can see.
///
...
/// to its origin.
[<Fact(Timeout = 15000)>]
let ``a deps manifest older than its restore is STALE though every assembly is current`` () =
    withTempDir "tp-superseded" (fun tmpDir ->
...
/// ignore it.
[<Fact(Timeout = 15000)>]
let ``a deps manifest at or after its restore is fresh`` () =
    withTempDir "tp-control" (fun tmpDir ->
        let projDir = p [ tmpDir; "Unit" ]
        let tfmDir = p [ projDir; "bin"; "Debug"; "net10.0" ]
...
[<Fact(Timeout = 15000)>]
let ``neither half of the pair missing is judged as stale`` () =
    withTempDir "tp-absent" (fun tmpDir ->
        let projDir = p [ tmpDir; "Unit" ]
        let tfmDir = p [ projDir; "bin"; "Debug"; "net10.0" ]
        let assets = p [ projDir; "obj"; "project.assets.json" ]
        let manifest = p [ tfmDir; "Unit.deps.json" ]
...
// second half — CONTENT FILES, which let a red main through. A shared
...
// The closure-parse channel was fail-closed from the start; the WALK
...
// =============================================================================
// the same wolf-cry through a different door: not the wrong project this
...
[<Fact(Timeout = 30000)>]
let ``a rerun queued for debt the active run clears preserves that run's evidence`` () =
...
[<Fact(Timeout = 5000)>]
let ``THE OTHER DIRECTION — a real mass failure is still RED, not an abort`` () =
...
[<Fact(Timeout = 5000)>]
let ``a SIGABRTed host is an abort even when it wrote a CLEAN report`` () =
...
[<Fact(Timeout = 5000)>]
let ``the dirty-shutdown flake (exit 7) is STILL green — no regression`` () =
...
[<Fact(Timeout = 5000)>]
let ``an abort report never counts the killed run's transcript as failures`` () =
...
[<Fact(Timeout = 5000)>]
let ``an aborted project is a HostAborted ledger entry, and a failed one still Error`` () =
...
[<Fact(Timeout = 20000)>]
let ``BatchChecked drains a pending queue instead of resting on a stale verdict`` () =
...
[<Fact(Timeout = 20000)>]
let ``an UNREADABLE ledger widens to the FULL suite rather than greening on nothing`` () =
...
[<Fact(Timeout = 20000)>]
let ``a MISSING ledger (fresh clone) is legitimately empty and does NOT force a full suite`` () =
...
[<Fact(Timeout = 20000)>]
let ``a genuinely EMPTY ledger stays a fast no-op (not a widened run)`` () =
...
// --- an unanalysable file must not vanish from the impact graph ---
//
...
// --- the full-suite scope is part of the task cache key ---
...
[<Fact(Timeout = 10000)>]
let ``cacheKeyFor refuses BuildCompleted while the process has NO test evidence`` () =
...
[<Fact(Timeout = 10000)>]
let ``the TestsFinished WRITE is not gated on session evidence`` () =
...
// --- `confirm`'s scope is a PROJECTION of RunCoverage ---
//
...
[<Fact(Timeout = 20000)>]
let ``a DISJOINT impact-filtered green does NOT clear a failed project's red`` () =
...
[<Fact(Timeout = 20000)>]
let ``a COVERING impact-filtered green DOES clear the red (no stuck-red)`` () =
    ...
    // is a different bug.
    withTempDir "covering-receipt" (fun repoRoot ->
        let handler =
...
[<Fact(Timeout = 20000)>]
let ``an unfiltered re-run (test-rerun) clears an outstanding red`` () =
...
[<Fact(Timeout = 20000)>]
let ``a filtered green over a DIFFERENT class in the same project does not clear it`` () =
...
[<Fact(Timeout = 20000)>]
let ``the zero-affected skip (0 ran, green) cannot launder an outstanding red`` () =
    ...
    // test below pins. Neither may be a green.
    match lastStatus statuses with
    | PluginStatus.Failed _ -> ()
    | other -> Assert.Fail($"a run that executed nothing must not launder an outstanding red, got %A{other}")

[<Fact(Timeout = 20000)>]
let ``a run that executed NO project records a verdict that says nothing was verified`` () =
    // The same degenerate skip lifecycle as the test above, minus the
...
[<Fact(Timeout = 20000)>]
let ``a run that DID execute keeps its counts verdict`` () =
...
[<Fact(Timeout = 20000)>]
let ``a run whose test HOST DIED completes as an ABORT, never as "N failed"`` () =
...
[<Fact(Timeout = 20000)>]
let ``THE OTHER DIRECTION — a run with a REAL failure still fails, and names the abort apart`` () =
...
[<Fact(Timeout = 20000)>]
let ``a TIMED-OUT project's red needs a WHOLE-project pass, not a class-filtered one`` () =
...
let ``RunCoverage: a raw --filter passthrough with NO report evidence claims no coverage`` () =
    ...
    // claim nothing. This is the floor builds on, not a replacement for it.
    let coverage =
        RunCoverage.ofRun (Map.ofList [ "ProjA", ProjectInFull ]) (Map.ofList [ "ProjA", passed true ]) noReportEvidence

    test <@ coverage = Map.empty @>

// --- a green re-run retires the red it just disproved ---
...
let ``d223: a quarantined method passing in the exact CTRF receipt retires its red`` () =
    withTempDir "a67-d223-reconcile" (fun repoRoot ->
        ...
        let methodName =
            "an unthrottled request answers the same for a real address and an unknown one"

        let fullName = $"%s{className}.%s{methodName}"

        let handler =
            create ":memory:" repoRoot (Some [ projConfig project ]) None None None None []
...
[<Fact(Timeout = 20000)>]
let ``a different passing method in the quarantined class cannot erase the red`` () =
    withTempDir "method-receipt" (fun repoRoot ->
        let project = "Intelligence.Tests.Integration"
...
[<Fact(Timeout = 10000)>]
let ``a report's PASSED classes are the receipt a raw-filter run is credited with`` () =
    let report =
        ctrfReport
...
[<Fact(Timeout = 10000)>]
let ``recall denominator requires every CTRF failed row promised by the summary`` () =
    let complete =
        ctrfReport
...
[<Fact(Timeout = 15000)>]
let ``non-executing and shared-infrastructure runs cannot produce exact recall evidence`` () =
    withTempDir "typed-run-evidence" (fun repoRoot ->
        let runId = Guid.NewGuid()
...
[<Fact(Timeout = 15000)>]
let ``one typed infrastructure exception invalidates a mixed failure sample`` () =
    let mixed =
...
[<Fact(Timeout = 15000)>]
let ``infrastructure tokens in names and assertion messages are not exception evidence`` () =
    let assertionOnly =
...
[<Fact(Timeout = 15000)>]
let ``structured exception objects and stack arrays carry infrastructure evidence`` () =
    let structured =
...
[<Fact(Timeout = 15000)>]
let ``an assertion trace naming a SocketException test class is not infrastructure evidence`` () =
    let ordinaryTrace =
...
[<Fact(Timeout = 15000)>]
let ``longer identifiers containing exception type names are not infrastructure evidence`` () =
...
[<Fact(Timeout = 15000)>]
let ``malformed parseable CTRF structure becomes unknown evidence`` () =
    withTempDir "malformed-structure" (fun repoRoot ->
        let runId = Guid.NewGuid()
        let project = "Database.Tests"
        let dir = Path.Combine(repoRoot, ".fshw", "test-runs", runId.ToString("N"))
        Directory.CreateDirectory dir |> ignore
...
[<Fact(Timeout = 20000)>]
let ``completion publishes real CTRF recall through check-reach IPC`` () =
    withTempDir "recall-ipc" (fun repoRoot ->
...
[<Fact(Timeout = 10000)>]
let ``a class that RAN AND FAILED is claimed by nothing, not even its own passes`` () =
...
[<Fact(Timeout = 10000)>]
let ``a class with nothing but skips proves nothing; a skip beside a pass is neutral`` () =
    let allSkipped =
...
[<Fact(Timeout = 10000)>]
let ``report evidence that is missing, unparseable or INCOMPLETE claims nothing`` () =
...
[<Fact(Timeout = 10000)>]
let ``a raw --filter run is credited with the classes its own report shows passing`` () =
...
[<Fact(Timeout = 10000)>]
let ``report evidence never speaks for a project the run did not LAUNCH`` () =
    // The laundering vector re-checked against report evidence: ProjA was
...
[<Fact(Timeout = 10000)>]
let ``a TIMED-OUT project is credited with nothing, report or no report`` () =
...
[<Fact(Timeout = 20000)>]
let ``a filtered re-run that PASSES retires the red it re-ran — and only that one`` () =
    ...
    let repoRoot =
        Path.Combine(Path.GetTempPath(), "fshw-" + Guid.NewGuid().ToString("N"))
...
[<Fact(Timeout = 20000)>]
let ``a filtered re-run whose report is missing or garbage retires NOTHING`` () =
    // Fail closed at the top of the stack too: a defect here must cost a stuck red, never
    // a false green.
    let repoRoot =
        Path.Combine(Path.GetTempPath(), "fshw-closed-" + Guid.NewGuid().ToString("N"))
...
[<Fact(Timeout = 10000)>]
let ``browser red survives a following edit whose graph selects zero tests`` () =
    let project = "Intelligence.Tests.Integration"

    let browserClass =
        "Intelligence.Tests.Integration.Web.BrowserIntegrationTests+BrowserIntegrationTests"

    // The historical edit changed only browser-test waits. Its graph
...
[<Fact(Timeout = 10000)>]
let ``no cache participation while a red is outstanding`` () =
...
[<Fact(Timeout = 10000)>]
let ``confirm still rejects a filtered green as UnearnedScope`` () =
...
[<Fact(Timeout = 10000)>]
let ``x 129: a RAW-filter run with no report evidence claims NO coverage, so the gate sees no scope at all``
    ()
    =
...
[<Fact(Timeout = 10000)>]
let ``x 112: a raw-filter run WITH evidence is a FILTERED scope, and confirm still refuses it`` () =
...
[<Fact(Timeout = 20000)>]
let ``a test run does not erase the unanalysable-file warning`` () =
...
[<Fact(Timeout = 20000)>]
let ``the last run's coverage is readable from state (a verdict writer's receipt)`` () =
...
let ``test-scope declares EVERY run the session completed, not only the one the receipt names`` () =
    // The receipt deliberately holds ONE run — the full-suite one, which
...
[<Fact(Timeout = 40000)>]
let ``POSITIVE CONTROL — a fresh tree runs every suite`` () =
...
[<Fact(Timeout = 40000)>]
let ``a stale project refuses the run BEFORE any suite executes`` () =
...
[<Fact(Timeout = 40000)>]
let ``a stale build-output COPY is repaired and the run proceeds`` () =
...
[<Fact(Timeout = 40000)>]
let ``an unrepairable stale tree reaches the CLI as StaleOutput, not as a build-ordering defer`` () =
...
[<Fact(Timeout = 15000)>]
let ``an apphost-missing defer still classifies as a build-ordering wait`` () =
    let deferred: TestResults =
...
// ---------------------------------------------------------------------------
// TEST-PRUNE's own cold-vs-cached ledger parity
// ---------------------------------------------------------------------------

[<Fact(Timeout = 60000)>]
let ``a cached test-prune replay clears the whole ledger, exactly as its real run does`` () =
...
let private seamProjects =
    [ testConfigNamed "Alpha.Tests"
      testConfigNamed "Beta.Tests"
      testConfigNamed "Gamma.Tests" ]

let private seamFailure (project: string) (cls: string option) : OutstandingFailure =
    { Project = project
      Class = cls
      Method = cls |> Option.map (fun _ -> "failed method")
      File = "tests/X.fs"
...
let ``the would-have-run selection drops the FULL-SUITE widening and keeps every other one`` () =
    ...
    let selection =
        wouldHaveRunSelection seamProjects symbolAffected Set.empty Set.empty false

    test <@ selection = Map.ofList [ "Alpha.Tests", ProjectClasses(Set.ofList [ "Alpha.OneTests" ]) ] @>

    // The coarse fallback for unanalysable files fires in the inner loop
    // too, so it SURVIVES the projection — as a whole-project run, which is what it is.
    let withCoarse =
        wouldHaveRunSelection seamProjects symbolAffected (Set.ofList [ "Beta.Tests" ]) Set.empty false
    ...
    // An UNREADABLE pending-verification ledger widens `check` to the
    // whole suite as well, and must therefore widen the projection.
    let withUnreadableLedger =
        wouldHaveRunSelection seamProjects symbolAffected Set.empty Set.empty true
...
let ``the would-have-run selection retains a runtime-only project's reach`` () =
    ...
    let reach =
        CheckReach.classify (Some selection) [ seamFailure "Beta.Tests" (Some "Beta.RuntimeOnlyTests") ]

    test <@ reach = ReachedAFailure [ "Beta.Tests" ] @>

[<Fact(Timeout = 15000)>]
let ``scopeOfSelection describes what check would have covered, and never rounds up`` () =
    let projects = seamProjects |> List.map (fun c -> c.Project)

    test <@ scopeOfSelection projects Map.empty = ScopeNone 3 @>

    test
        <@
            scopeOfSelection projects (seamProjects |> List.map (fun c -> c.Project, ProjectInFull) |> Map.ofList) = ScopeFull
                3
        @>
...
let ``CheckReach.classify decides reach per failure, and refuses what it cannot decide`` () =
    ...
    let projectMiss =
        CheckReach.classify (Some inFull) [ seamFailure "Beta.Tests" (Some "Beta.OneTests") ]
    ...
    test
        <@
            CheckReach.classify (Some inFull) [ seamFailure "Alpha.Tests" (Some "Alpha.TwoTests") ] = ReachedAFailure
                [ "Alpha.Tests" ]
        @>

    test <@ CheckReach.classify (Some inFull) [ seamFailure "Alpha.Tests" None ] = ReachedAFailure [ "Alpha.Tests" ] @>

    // A class-filtered project reaches the failure only when the class is in the filter.
    test
        <@
            CheckReach.classify (Some filtered) [ seamFailure "Alpha.Tests" (Some "Alpha.OneTests") ] = ReachedAFailure
                [ "Alpha.Tests" ]
        @>

    let classMiss =
        CheckReach.classify (Some filtered) [ seamFailure "Alpha.Tests" (Some "Alpha.TwoTests") ]
    ...
    match CheckReach.classify (Some filtered) [ seamFailure "Alpha.Tests" None ] with
    ...
    match
        CheckReach.classify
            (Some filtered)
            [ seamFailure "Alpha.Tests" (Some "Alpha.OneTests")
              seamFailure "Alpha.Tests" None ]
    with
    | ReachUnknown _ -> ()
...
let ``incomplete failure evidence cannot become a selection alarm`` reason =
...
[<Fact(Timeout = 15000)>]
let ``complete evidence preserves genuine mass selection misses`` () =
    let selection = Some(Map.ofList [ "Unit.Tests", ProjectInFull ])

    let failures =
        [ 1..600 ]
        |> List.map (fun index -> seamFailure "Database.Tests" (Some $"Database.Case%d{index}"))
...
[<Fact(Timeout = 15000)>]
let ``failure recall is an exact numerator over full-run failures and never vacuous`` () =
    let selection =
        Map.ofList
    ...
    let failures =
        [ seamFailure "Alpha.Tests" (Some "Alpha.OneTests")
          seamFailure "Beta.Tests" (Some "Beta.MissedTests") ]
...
[<Fact(Timeout = 15000)>]
let ``failure recall requires every observed failure to be decidable`` () =
    let inFull = Map.ofList [ "Alpha.Tests", ProjectInFull ]

    match CheckReach.measure (Some inFull) [ seamFailure "Alpha.Tests" None ] with
