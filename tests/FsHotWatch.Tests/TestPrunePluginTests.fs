...
// ---------------------------------------------------------------------------
// AUTOMATION-259 (rework) — the DAEMON SEAM, end to end.
//
...
let private a259SeamTree (tmpDir: string) =
    let dbPath = Path.Combine(tmpDir, "tp.db")
    let db = Database.create dbPath
    PendingQueueHelpers.seedCoveredSymbol db "Lib.foo" "Lib.fs" "P1" "P1Tests" "fooTest"
    PendingQueueHelpers.seedCoveredSymbol db "Lib.debt" "Debt.fs" "P2" "P2Tests" "debtTest"
...
let private a259CheckReachReply (host: PluginHost) : string =
...
[<Fact(Timeout = 25000)>]
let ``AUTOMATION-259: a confirm-style full-suite run SERVES the narrower selection check would have used`` () =
    withTempDir "tp-a259-seam-projected" (fun tmpDir ->
        let dbPath, p1Ran, p2Ran, configs = a259SeamTree tmpDir
        ...
        use doc = JsonDocument.Parse(a259CheckReachReply host)
...
[<Fact(Timeout = 25000)>]
let ``AUTOMATION-259: an impact-scoped run retains NO selection, and says so rather than inventing one`` () =
    ...
    withTempDir "tp-a259-seam-impact" (fun tmpDir ->
        let dbPath, p1Ran, p2Ran, configs = a259SeamTree tmpDir
