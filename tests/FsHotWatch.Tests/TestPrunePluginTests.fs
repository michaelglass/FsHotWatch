...
let ``incident: a beforeRun throw in the run-tests command surfaces as Failed, not a swallowed error`` () =
    // the gate-trust hole. The manual `run-tests` command ran
...
// =============================================================================
// the gate must CONVERGE, never rest on a
...
[<Fact(Timeout = 20000)>]
let ``a symbol covered only by an unconfigured test project drops instead of wedging the gate red`` () =
...
// --- the merge gate's scope is part of the §2a cache key ---
...
// --- the gate's scope is a PROJECTION of RunCoverage ---
//
...
// ---------------------------------------------------------------------------
// a test run the daemon cannot SEE is a gate that cannot
...
// --- MergeGate's UnearnedScope protection is untouched ---

[<Fact(Timeout = 10000)>]
let ``the merge gate still rejects a filtered green as UnearnedScope`` () =
