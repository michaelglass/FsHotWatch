...
///Tests that need a run to CLEAR something use
...
let ``FileChecked exception while tests running surfaces in the ledger without stomping the run's status (F10)``
    ...
    withTempDir "tp-f10-not-idle" (fun tmpDir ->
        ...
        //the run reported Running and still owns the status.
...
let ``test errors are cleared when all tests pass`` () =
    withTempDir "tp-ledger-clear" (fun tmpDir ->
        ...
        //
        // the second cycle is driven by `run-tests`, not by a second
...
// =============================================================================
// seeded-workspace under-selection regression tests.
//
...
// symbols → zero affected tests → vacuous green (the gate that
...
// QA: the "nothing to verify" completion path. A cycle whose
...
let ``all changed symbols with no covering test complete green without running`` () =
    withTempDir "tp-nothing-to-verify" (fun tmpDir ->
        ...
        // this DB compatibly. Both halves matter: the guarantee under
...
// The "nothing to verify" green above infers "this symbol has no
...
//
// The discriminator against the test above is that here the symbol IS
...
// the poisoned-seed guard's decision, tested without a daemon.
...
let ``a seed that is both pinned and dominant is a poison suspect`` () =
    ...
    // every run (— `name` alone selected 2,837 tests).
...
let ``DefaultRunTestsWaitMs is well above the old fixed 120s`` () =
    // Regression guard: the whole point of bug 2 is that the wait
...
// ---------------------------------------------------------------------------
// every project's output is STREAMED to
...
let ``ranFullSuite is FALSE for empty results — nothing ran, so nothing was proved`` () =
    // Reversed deliberately. See the twin in EventTests.
...
// =============================================================================
// a FAILED test verdict must never be served from the task cache.
...
// a run that matched NO tests must not be cacheable as a green.
//
...
// ---------------------------------------------------------------------------
// finding 6 — the cache key must not pay for inputs it discards.
// ---------------------------------------------------------------------------
//
...
let ``a failed test run is not cached, so a later run on the same key is a miss and reports green`` () =
    ...
    withTempDir "tp-fail-not-cached" (fun tmpDir ->
        ...
        //— and the question here is not "does the plugin participate",
...
//
// rebuilt WHAT it compares. The first cut compared the test DLL
...
// =============================================================================
// direction 1 — THE FALSE POSITIVE. An edit to a project OUTSIDE
...
// simulates. It used to move the origin's mtime alone, which
...
// =============================================================================
// second half — CONTENT FILES. This one let a RED main through.
//
...
// =============================================================================
// THE WOLF-CRY, BACK THROUGH A DIFFERENT DOOR. Not the wrong
...
module private PendingQueueHelpers =
    ...
    /// The tracked issue closes. The tests that WANT the unreadable case match on
...
let ``incident: a beforeRun throw in the run-tests command surfaces as Failed, not a swallowed error`` () =
    // the verdict-trust hole. The manual `run-tests` command ran
...
let ``incident: a test child that never becomes a live process drives the run to Failed, not a wedge`` () =
    // QA finding — the launch gap. Between a config's spawn and its
    ...
    // SAME Aborted lifecycle a beforeRun throw does (seam) →
...
// =============================================================================
// the check must CONVERGE, never rest on a
...
//
//   * a stale `Completed` → FALSE GREEN with symbols pending
//   * a stale `Failed`    → PERMANENTLY STUCK RED, work never runs
//
...
/// The two-project runner used by the tests. P1 covers `Lib.foo`,
...
let ``an UNREADABLE ledger widens to the FULL suite rather than greening on nothing`` () =
    ...
    withTempDir "tp-ledger-unreadable" (fun tmpDir ->
        ...
        // The cold-scan shape: BatchChecked is the cohort
...
// --- the full-suite scope is part of the §2a cache key ---
...
let ``cacheKeyFor: two full-suite runs over the same tree DO share a key`` () =
    ...
    // It is not sound. The key does not pin the TREE: on a cold scan
...
//
// These replace the `classifyRunScope` suite. That function derived
// `confirm`'s scope INDEPENDENTLY, from `LastResults`, while the ledger decided what a
// run may CLEAR from `RunCoverage` — two answers to one question,
...
let ``scopeOf: the zero-affected skip's empty green is NO SCOPE, not a full suite`` () =
    ...
    // the CLI refuses to call that green in EITHER mode.
...
// ---------------------------------------------------------------------------
// a test run the daemon cannot SEE is evidence it cannot
...
let ``a green run's Completed status carries its verdict`` () =
    // the type-level guarantee end-to-end: the status a real
...
// not decline it (review, finding 1).
// ---------------------------------------------------------------------------
...
let ``run-tests bounds its wait: a run that outlives the budget reports busy, never a verdict`` () =
    // the rule applied to the last unbounded seam: the reply wait.
...
// a verdict that was not earned, "no failures reported by
...
// green MUST (no over-correction into 's permanent stuck-red).
// =============================================================================
...
/// reads whenever a run wrote no readable/complete CTRF report. With
...
// a run in which EVERY project matched zero tests must not produce a
...
let ``a COVERING impact-filtered green DOES clear the red (no stuck-red)`` () =
    // The other direction — the over-correction guard (cf. the stuck-RED
...
let ``RunCoverage: a raw --filter passthrough with NO report evidence claims no coverage`` () =
    ...
    // either, we claim nothing rather than guess — the fail-closed floor the tracked issue
...
let ``report evidence never speaks for a project the run did not LAUNCH`` () =
    // the laundering vector, re-checked against the new input: ProjA was
...
let ``OutstandingFailure.carry: a covered project that failed AGAIN keeps exactly one red`` () =
    ...
    // Superseded by this run's own evidence — one entry, not two (the
...
// --- Confirmation's UnearnedScope protection is untouched ---
...
let ``x 112: a raw-filter run WITH evidence is a FILTERED scope, and confirm still refuses it`` () =
    ...
    // the ledger and the verdict read the same coverage, as the tracked issue requires.
    let configs = [ projConfig "ProjA"; projConfig "ProjB" ]
