...
let ``InnerLoop: NO TESTS RAN is never Clean — the inner loop may test LESS, not NOTHING`` () =
    // `(NoTestsRun NoTestsReason.Unstated)` does NOT mean "impact analysis selected nothing this
...
let ``parseTestRunReport: the session's run ledger comes through, newest first`` () =
    // The daemon DECLARES every run it has completed, because the CLI
...
[<Fact(Timeout = 15000)>]
let ``an all-unattributable ledger is NO VERDICT (exit 3), not a red`` () =
    let stale =
        { PluginStatuses = statusOf (StatusView.Completed DateTime.UtcNow)
...
[<Fact(Timeout = 15000)>]
let ``ONE attributable diagnostic among them keeps the red`` () =
...
[<Fact(Timeout = 15000)>]
let ``a FAILING PLUGIN beside a stale ledger keeps the red`` () =
...
[<Fact(Timeout = 15000)>]
let ``a CLEAN ledger is still Clean, never stale-daemon-state`` () =
...
[<Fact(Timeout = 15000)>]
let ``a stale-output deferral classifies apart from a build-ordering one`` () =
...
[<Fact(Timeout = 15000)>]
let ``the verdict carries the stale deferrals, still exit 2, still not a red`` () =
    let stale =
...
[<Fact(Timeout = 15000)>]
let ``a real failure beside a stale-output defer is still FailuresFound`` () =
    let both =
        { PluginStatuses = Map.empty
          FailingDiagnostics = 1
          UnattributableDiagnostics = 0
...
// ---------------------------------------------------------------------------
// a dead test host is exit 2, and a red is still exit 1.
// ---------------------------------------------------------------------------
...
[<Fact(Timeout = 15000)>]
let ``THE OTHER DIRECTION — a real failure beside an abort is still FailuresFound`` () =
...
[<Fact(Timeout = 15000)>]
let ``NoAbort changes nothing — a clean run is still Clean`` () =
...
[<Fact(Timeout = 15000)>]
let ``an abort DOMINATES a concurrent build defer`` () =
...
// ---------------------------------------------------------------------------
// a `Clean` cannot be minted without a baseline
// ---------------------------------------------------------------------------
...
[<Fact>]
let ``an ABSENT baseline is NO VERDICT (exit 3) carrying the daemon's reason, never a green`` () =
    let absent =
...
[<Fact>]
let ``a daemon that did not REPORT a baseline is refused, with this build's words`` () =
    let silent =
        inputs false Complete (ImpactFiltered(1, 4))
...
[<Fact>]
let ``a repo with NO test suite is clean relative to nothing, and only when its scope is unknown`` () =
    let none =
...
[<Fact>]
let ``failures, aborts and deferrals outrank a missing baseline`` () =
...
// ----------------------------------------------------------------------------
// A run that verified nothing is `RunOutcome.VerifiedNothing` on a
// `Completed` status. It must not count as a failed plugin: a zero-project check
// exits 3 (NO VERDICT), never 1 (failures found).
// ----------------------------------------------------------------------------

[<Fact(Timeout = 15000)>]
let ``a Completed plugin whose run VERIFIED NOTHING is not a failed plugin — the check exits 3, never 1`` () =
