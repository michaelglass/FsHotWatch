...
//that what it could NOT see is reported rather than deleted.
...
// ---------------------------------------------------------------------------
// The skip channel. `bestEffortFiles` DISCARDS what the walk
...
//
// `ProjectGraph` is handed MSBuild's compile items, which include
...
let ``SourceExcludedDirs is stated in terms of BuildOutputDirs`` () =
    ...
    // exactly the drift cost two releases to find.
