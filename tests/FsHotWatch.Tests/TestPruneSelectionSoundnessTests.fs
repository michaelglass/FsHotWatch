...
[<Fact(Timeout = 60000)>]
let ``replay: a red test the next change does not reach is still selected, and the verdict stays red until it passes``
    ()
    =
    withTempDir "quarantine" (fun tmpDir ->
...
[<Fact(Timeout = 60000)>]
let ``a red survives a daemon restart and is quarantined into a FILTERED first run`` () =
    ...
    withTempDir "restart" (fun tmpDir ->
        let dbPath = twoProjectDb tmpDir
        ...
        // pre state on restart) and queue the same change: the filtered
...
[<Fact(Timeout = 60000)>]
let ``an UNREADABLE outstanding-failures ledger widens to the full suite rather than forgetting the reds`` () =
    withTempDir "reds-unreadable" (fun tmpDir ->
        let dbPath = twoProjectDb tmpDir
...
[<Fact(Timeout = 60000)>]
let ``with no full-suite baseline a filtered selection widens to the full suite and earns one`` () =
    withTempDir "no-baseline" (fun tmpDir ->
        let dbPath = twoProjectDb tmpDir
...
[<Fact(Timeout = 60000)>]
let ``with a valid baseline the same queued change runs the filtered selection only`` () =
    // The positive control for the widening above: the ONLY difference is the baseline.
    withTempDir "baseline-valid" (fun tmpDir ->
        let dbPath = twoProjectDb tmpDir
        seedBaseline tmpDir [ "P1"; "P2" ]
...
[<Fact(Timeout = 60000)>]
let ``a baseline that never executed a newly configured project is STALE and widens`` () =
    withTempDir "baseline-stale" (fun tmpDir ->
        let dbPath = twoProjectDb tmpDir
...
[<Fact(Timeout = 60000)>]
let ``a red full suite still records the baseline, because its reds are carried`` () =
    ...
    withTempDir "red-baseline" (fun tmpDir ->
        let dbPath = twoProjectDb tmpDir
...
[<Fact(Timeout = 60000)>]
let ``test-scope says why there is no baseline, then names the run that earned it`` () =
    withTempDir "scope-baseline" (fun tmpDir ->
        let dbPath = twoProjectDb tmpDir
...
let ``a symbol with no covering test anywhere is uncovered, and names no unrunnable project`` () =
    ...
    withTempDir "uncovered" (fun tmpDir ->
        let dbPath = Path.Combine(tmpDir, "tp.db")
        let db = Database.create dbPath
        PendingQueueHelpers.seedCoveredSymbol db "Lib.foo" "Lib.fs" "P1" "P1Tests" "fooTest"
...
let ``FullSuiteBaseline round-trips, reads a missing file as none, and an existing unreadable one as Unreadable`` () =
    withTempDir "baseline-file" (fun tmpDir ->
...
let ``OutstandingFailure ledger round-trips without the detail, and a torn file is Unreadable`` () =
    withTempDir "reds-file" (fun tmpDir ->
