...
// the daemon/CLI binary-identity handshake. A new CLI must
// never silently talk to an old daemon — the trap (a label
...
let ``compareIdentity is stale when the content hash differs even with the same version`` () =
    // LITERALLY the tracked issue: a locally-repacked build shares the version
