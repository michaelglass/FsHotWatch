...
// ---------------------------------------------------------------------------
// FILE-COMMAND's own cold-vs-cached ledger parity
// ---------------------------------------------------------------------------

[<Fact(Timeout = 30000)>]
let ``a cached file-command replay leaves an out-of-batch finding standing`` () =
...
// exercise a real command and task-cache replay before projecting
...
let ``real cold replay and changed-input plugin results expose structured provenance`` fails =
    withTempDir "plugin-provenance" (fun root ->
...
let ``configured decline exit is distinct from pass and ordinary failure in the verdict`` configured exitCode expectedOutcome =
    withTempDir "decline-command" (fun root ->
