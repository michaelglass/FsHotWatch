...
/// "full suite". Since the tracked issue the results are the single input, so a test
...
let ``afterTests command does not fire for a run that executed nothing`` () =
    ...
    // lifecycles emitted before the tracked issue made them say `false`. Keeping the
...
let ``FullSuiteClaim is unknown when every project in a completed run failed to execute`` () =
    ...
    // did not exist before the tracked issue and had to be re-derived by each consumer.
    let verification =
        verificationFor
