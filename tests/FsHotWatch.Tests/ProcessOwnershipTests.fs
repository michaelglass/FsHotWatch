...
let ``same registered cleanup capability can settle after an audited failed attempt`` () =
    ...
    use ownedProcess = Process.GetCurrentProcess()
    let parent = ProcessRegistry.Registry()
    use installed = ProcessRegistry.install parent
    ...
    ProcessRegistry.withChildScope CancellationToken.None (fun settle ->
        ProcessRegistry.trackOwned ownedProcess terminate
        let firstOutcome =
            ProcessHelper.killTreeWith (TimeSpan.FromSeconds 2.) ownedProcess.Id (fun () -> "recovery capability fixture") terminate
        match firstOutcome with
        | ProcessHelper.KillOutcome.KillFailed error -> Assert.Same(firstFailure, error)
        | other -> Assert.Fail $"Expected the original failed attempt, got {other}"
        Assert.Single(ProcessRegistry.leaked()) |> ignore
        ProcessHelper.settleOwnedProcess
            None
            terminate
            (fun error -> ProcessRegistry.reportLeak ownedProcess.Id "recovery capability fixture" error.Message)
            (fun () ->
                released <- true
                ProcessRegistry.untrack ownedProcess)
        Assert.True(released)
    ...
    let failure =
        Assert.Throws<InvalidOperationException>(Action(fun () ->
            ProcessRegistry.withChildScope CancellationToken.None (fun settle ->
                ...
                | ProcessHelper.KillOutcome.KillFailed error -> Assert.Same(refusal, error)
