...
let ``AUTOMATION-474 quiet convergence refuses evidence after an exact-tree-only edit`` (declaredInput: bool) =
    TestHelpers.withTempDir "a474-exact-tree-convergence" (fun repoRoot ->
        ...
        System.IO.Directory.CreateDirectory(System.IO.Path.GetDirectoryName source) |> ignore
        ...
        let getErrors () =
            errorReads <- errorReads + 1
            if errorReads = 1 then """{"count":0,"files":{},"statuses":{},"unchecked":1}"""
            else """{"count":0,"files":{},"statuses":{},"unchecked":0}"""
        ...
        let rescan () =
            rescans <- rescans + 1
            if declaredInput then System.IO.File.WriteAllText(policy, "floor=90\n")
            else System.IO.File.AppendAllText(source, "// exact tree changed without changing symbol bodies\n")
            "idle"
