...
// AUTOMATION-555. This is the real parse -> publish -> persisted verdict path.
...
let ``daemon clock quality cannot disappear before verdict publication`` (scenario: string) =
    TestHelpers.withTempDir "ipcoutput-a555-clock-quality" (fun repoRoot ->
        let runId = System.Guid.NewGuid()
