...
[<Fact(Timeout = 15000)>]
let ``the packed CLI uses the exact restore and build checked by the dependency guard`` () =
    let workflow =
        File.ReadAllLines(
            Path.GetFullPath(
...
[<Fact(Timeout = 15000)>]
let ``the structure list covers every MSBuild implicit import, not just Directory.Build.props`` () =
...
[<Fact(Timeout = 15000)>]
let ``implicitImportsFor finds the NEAREST ancestor, MSBuild's own rule`` () =
...
[<Fact(Timeout = 15000)>]
let ``implicitImportsFor ignores a props file in a SIBLING directory`` () =
