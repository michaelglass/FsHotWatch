...
type private Spec =
    {
        ...
        /// The batches, when a test is about a check that ran the tests
        ...
        /// The failing ledger diagnostics behind a red. Defaults empty.
        ...
        SeedTotal: int
        /// The declared, reasoned gaps in this run's scope.
        ...
        /// The project model the verdict was graded against. Defaults to
        /// the shared available fixture — the only reading a green may record.
...
// ---------------------------------------------------------------------------
// Hook attribution: named, timed steps beside the plugin records,
...
let ``a changed CONTENT fixture makes the verdict stale — the dsa-scope-4 false green`` () =
    // `dsa-scope-4.json` changed (36 -> 40 leaf facts), MSBuild deemed the
...
let ``a truncated verdict file is Unreadable, never a green`` () =
    withTempDir "verdict-torn" (fun root ->
        ...
        File.WriteAllText(
            Verdict.path root,
            """{"schema":"fshw-verdict-v2","projectModel":{"schema":"fshw-project-model-v1","status":"available","generation":7,"counts":{"discovered":3,"loaded":3,"optionsMapped":3,"registered":3},"reasonCode":null},"outcome":{"kind":"gr"""
        )
...
let ``a confirm verdict round-trips as a CONFIRM — the writer and the reader agree on the token`` () =
    // The `command` field is a WIRE token: `Verdict.write` emits
...
[<Fact>]
let ``a killed host serializes as incomplete with an ABORT reason, never red`` () =
...
let ``a verdict written before runs[] existed rehydrates as ONE batch, named by the run it was graded from`` () =
    ...
    withTempDir "runs-legacy" (fun root ->
        ...
        let legacy =
            $$"""{ "schema": "fshw-verdict-v2", "projectModel":{"schema":"fshw-project-model-v1","status":"available","generation":7,"counts":{"discovered":3,"loaded":3,"optionsMapped":3,"registered":3},"reasonCode":null}, "command": "check", "runId": "{{dir}}",
...
[<Fact>]
let ``tidyRunsDir rotates old RUNS and purges the pre flat layout`` () =
...
/// finishing the workflow-neutral pass the no-verdict hints already got.
...
/// principle `ScopeUnknown`/`ScopeUnreadable` already encode:
...
let ``every scope round-trips through the file`` () =
    withTempDir "verdict-scopes" (fun root ->
        ...
        // record an `ImpactFiltered`, so a confirm fixture could not
...
let ``every plugin outcome round-trips — and an unrecognized one is FAIL, not ok`` () =
    withTempDir "verdict-plugins" (fun root ->
        ...
        File.WriteAllText(
            Verdict.path root,
            """{"schema":"fshw-verdict-v2","projectModel":{"schema":"fshw-project-model-v1","status":"available","generation":7,"counts":{"discovered":3,"loaded":3,"optionsMapped":3,"registered":3},"reasonCode":null},"treeHash":"sha256:x","outcome":{"kind":"red"},
...
// ---------------------------------------------------------------------------
// a CONFIRM verdict may not carry an impact-filtered scope.
// ---------------------------------------------------------------------------
...
let ``a legacy confirm+filtered verdict FILE is UNREADABLE — it is re-earned, never migrated`` () =
    ...
    withTempDir "verdict-legacy-confirm-filtered" (fun root ->
        ...
        let legacy (command: string) =
            File.WriteAllText(
                Verdict.path root,
                $$"""{"schema":"fshw-verdict-v2","projectModel":{"schema":"fshw-project-model-v1","status":"available","generation":7,"counts":{"discovered":3,"loaded":3,"optionsMapped":3,"registered":3},"reasonCode":null},"treeHash":"sha256:x","command":"{{command}}",
...
// ---------------------------------------------------------------------------
// the check-vs-confirm sample a `confirm` leaves behind.
// ---------------------------------------------------------------------------
...
// =============================================================================
// the file and the exit code are ONE decision
// =============================================================================
...
let ``an escalated confirm records what the impact-scoped run concluded, and that they AGREED`` () =
    withTempDir "verdict-259-agreed" (fun root ->
        ...
        // may not say it, and the counts now live here rather than in
        // anyone's prose.
...
let ``check green + confirm red records CHECK MISSED FAILURES — the fshw defect, named`` () =
    withTempDir "verdict-259-missed" (fun root ->
        // this is not a merge saved, it is a SELECTOR BUG — `check` told
...
let ``an escalated run that never completed records COULD-NOT-COMPARE, never agreement`` () =
    withTempDir "verdict-259-incomparable" (fun root ->
        // The observed artifact: confirm escalated, the forced full run
...
let ``a verdict written before the check comparison still reads — as NOT RECORDED, never as agreement`` () =
    withTempDir "verdict-259-legacy" (fun root ->
        ...
        File.WriteAllText(
            Verdict.path root,
            """{"schema":"fshw-verdict-v2","projectModel":{"schema":"fshw-project-model-v1","status":"available","generation":7,"counts":{"discovered":3,"loaded":3,"optionsMapped":3,"registered":3},"reasonCode":null},"treeHash":"sha256:x","command":"confirm",
...
let ``an outcome this build cannot read is Unreadable, never a green`` () =
    withTempDir "verdict-badoutcome" (fun root ->
        ...
        File.WriteAllText(
            Verdict.path root,
            """{"schema":"fshw-verdict-v2","projectModel":{"schema":"fshw-project-model-v1","status":"available","generation":7,"counts":{"discovered":3,"loaded":3,"optionsMapped":3,"registered":3},"reasonCode":null},"treeHash":"sha256:x","outcome":{"kind":"chartreuse"}}"""
        )
...
let ``an incomplete verdict with no recorded reason still reads as incomplete`` () =
    withTempDir "verdict-noreason" (fun root ->
        ...
        File.WriteAllText(
            Verdict.path root,
            """{"schema":"fshw-verdict-v2","projectModel":{"schema":"fshw-project-model-v1","status":"available","generation":7,"counts":{"discovered":3,"loaded":3,"optionsMapped":3,"registered":3},"reasonCode":null},"treeHash":"sha256:x","outcome":{"kind":"incomplete"},"exitCode":2}"""
        )
...
[<Fact>]
let ``a verified-nothing plugin verdict is worded from the case when the record has no summary`` () =
...
[<Fact>]
let ``an IDLE plugin whose last run VERIFIED NOTHING is a WARN, never ok and never fail`` () =
...
let ``an UNREADABLE DIRECTORY fails the hash CLOSED — a hole is not an empty tree`` () =
    // `ContentHash` already made an unreadable FILE hash to a sentinel,
...
// current — a hole in the middle of the provenance chain. (makes the same
...
let ``a verdict with NO producer recorded does not apply — provenance unestablished`` () =
    withTempDir "verdict-noproducer" (fun root ->
        ...
        writeRaw
            root
            $$"""{"schema":"fshw-verdict-v2","projectModel":{"schema":"fshw-project-model-v1","status":"available","generation":7,"counts":{"discovered":3,"loaded":3,"optionsMapped":3,"registered":3},"reasonCode":null},"treeHash":"{{tree.Hash}}","outcome":{"kind":"green","baseline":{"kind":"full-suite-run","runId":"b0000000110040008000000000000110","earnedAt":"2026-09-06T12:00:00.0000000Z","projects":1} },"exitCode":0}"""
...
// ---------------------------------------------------------------------------
// `confirm` HONOURS a verdict it has already earned.
//
...
[<Fact>]
let ``a full-suite green over THIS tree, from THIS binary, still applies`` () =
...
[<Fact>]
let ``a verdict from a DIFFERENT fshw binary is never satisfied`` () =
...
[<Fact>]
let ``an impact-filtered green is NOT the claim confirm makes`` () =
    ...
    //
    // The subject is a CHECK verdict, and that is the whole point: a
...
[<Fact>]
let ``a RED verdict does not short-circuit — confirm re-runs and reports it`` () =
...
[<Fact>]
let ``no verdict on disk means earn one — a fresh checkout is not green`` () =
...
[<Fact>]
let ``Verdict.create refuses a red with no failing plugin and no structural cause`` () =
    let construct () =
        build
            { greenVerdict "deadbeef" 10 with
                Outcome = Verdict.Red
...
// ---------------------------------------------------------------------------
// case 3 — the verdict must explain its own exit code
// ---------------------------------------------------------------------------
//
...
[<Fact>]
let ``a persisted unexplained red degrades to incomplete`` () =
...
[<Fact>]
let ``a flood of causes is truncated but its true count is not`` () =
...
[<Fact>]
let ``a GREEN verdict names no causes`` () =
...
[<Fact>]
let ``AC5: an FCS internal error is a CHECKER FAULT, not a finding`` () =
...
[<Fact>]
let ``AC5: an ORDINARY fcs error against a present file stays a red`` () =
...
[<Fact>]
let ``AC5: a diagnostic against a file that is GONE is not about this tree`` () =
...
[<Fact>]
let ``AC5: a synthetic or relative ledger key is never demoted`` () =
...
[<Fact>]
let ``AC5: an internal-error message from a PLUGIN is not a checker fault`` () =
...
[<Fact>]
let ``AC5: the verdict file records each cause's kind`` () =
...
[<Fact>]
let ``AC5: the steering block names fshw stop for a cause that is not this tree`` () =
...
[<Fact>]
let ``AC5: an ordinary red says none of that`` () =
...
// ---------------------------------------------------------------------------
// the message an operator reads LAST, and acts on.
// ---------------------------------------------------------------------------
...
[<Fact>]
let ``the stale-output message names EVERY affected project, untruncated`` () =
...
[<Fact>]
let ``a build-ordering defer keeps the words that are true for IT`` () =
...
[<Fact>]
let ``the verdict file's reason carries the stale-output remedy`` () =
...
// ---------------------------------------------------------------------------
// a recall miss must SHOUT, in the output already being read
// ---------------------------------------------------------------------------
//
...
[<Fact>]
let ``a recall miss is named as a SELECTION BUG in the verdict output`` () =
    let hints =
        hintsFor
...
[<Fact>]
let ``verdict output reports the persisted conditional recall fraction and threshold`` () =
    let comparison =
...
[<Fact>]
let ``zero-denominator recall is rendered as not measurable, never perfect`` () =
    let comparison =
...
[<Fact>]
let ``the recall alarm is the FIRST thing in the block`` () =
...
[<Fact>]
let ``no recall alarm when the two readings agreed`` () =
...
// ---------------------------------------------------------------------------
// the declared gaps travel INSIDE `scope`
// ---------------------------------------------------------------------------
...
[<Fact>]
let ``named selection misses survive the verdict file and render as actionable evidence`` () =
    withTempDir "missed-roundtrip" (fun root ->
        makeRepo root
        let tree = TreeHash.compute root []
...
[<Fact>]
let ``projected confirm evidence carries conditional recall into the verdict comparison`` () =
    let reading =
        ReachRecorded
            { RunId = Some projectionRunId
              Scope = ImpactFiltered(2, 6)
...
[<Fact>]
let ``escalated confirm evidence also carries full-run conditional recall`` () =
...
[<Fact>]
let ``projected recall from another run is not measurable`` () =
    let reading =
        ReachRecorded
...
[<Fact>]
let ``executed recall without the graded run id is not measurable`` () =
    let executed: Verdict.ImpactScopedRun =
        { Scope = ImpactFiltered(2, 6)
          Outcome = Verdict.Green BaselineFixtures.baseline
          FailingSuites = []
...
[<Fact>]
let ``run-bound recall survives comparison creation and the verdict wire`` () =
...
let ``the selection missing the failure is CHECK-MISSED-FAILURES, which is the whole point`` () =
    // caught on the tree that produced it: the suite is red, and the
...
// ---------------------------------------------------------------------------
// the tree hash covers what DECIDES the check, not just the source
//
...
let ``Directory.Build.props moves the tree hash with NOTHING declared — the tool knows its own toolchain inputs`` () =
    ...
    withTempDir "toolknown" (fun root ->
        makeRepo root
...
let ``a declared DIRECTORY may POINT AT build output but never SWEEPS into it`` () =
    ...
    withTempDir "dir-vs-bin" (fun root ->
        makeRepo root
...
let ``a verdict from an OLDER hashing scheme is inapplicable by ALGORITHM — not puzzled over as a stale tree`` () =
    ...
    withTempDir "algorithm" (fun root ->
        makeRepo root
        let tree = TreeHash.compute root []
...
let ``an UNREADABLE .fshw.json declares nothing — and the hash still fails closed on it`` () =
    ...
    if not (OperatingSystem.IsWindows()) then
        withTempDir "unreadable-config" (fun root ->
            makeRepo root
...
let ``a repo root that cannot be listed contributes no tool-known inputs rather than throwing`` () =
    ...
    if not (OperatingSystem.IsWindows()) then
        withTempDir "blind-root" (fun root ->
...
// ---------------------------------------------------------------------------
// A verdict read MID-RUN must never return the previous run's green.
//
...
let ``a verdict that cannot say which run made it is refused while any run is in flight`` () =
    // Written before attribution existed, or by a path that could not
...
// ---------------------------------------------------------------------------
// a green names the baseline it is relative to, on disk and back
// ---------------------------------------------------------------------------

[<Fact>]
let ``a green round-trips its full-suite baseline through the verdict file`` () =
    withTempDir "verdict-roundtrip" (fun root ->
        makeRepo root
...
[<Fact>]
let ``a test-less repo's green round-trips as relative to no test suite`` () =
    withTempDir "verdict-no-suite" (fun root ->
        makeRepo root
...
let ``a green that names NO baseline is not a verdict this build can read`` () =
    ...
    withTempDir "verdict-legacy-green" (fun root ->
        ...
        File.WriteAllText(
            Verdict.path root,
            """{"schema":"fshw-verdict-v2","projectModel":{"schema":"fshw-project-model-v1","status":"available","generation":7,"counts":{"discovered":3,"loaded":3,"optionsMapped":3,"registered":3},"reasonCode":null},"treeHash":"sha256:x","outcome":{"kind":"green"},"exitCode":0,"plugins":[]}"""
        )
...
[<Fact>]
let ``a missing baseline reaches the verdict file as INCOMPLETE naming the baseline, never as red or green`` () =
    let outcome =
...
[<Fact>]
let ``the projected check reading is green RELATIVE TO the daemon's baseline, and refused without one`` () =
...
// tied to the outcome in both directions, and read back fail-closed.
// ---------------------------------------------------------------------------

let private rediscovering =
    ProjectModelReading.Observed(ProjectModel.Observation.Rediscovering 12L)

let private notReported =
...
[<Theory>]
[<InlineData("rediscovering")>]
[<InlineData("unobserved")>]
[<InlineData("registration-failed")>]
[<InlineData("not-reported")>]
let ``create REFUSES a green graded against a project model that was not available`` (model: string) =
    let reading =
        match model with
        | "rediscovering" -> rediscovering
        | "unobserved" -> ProjectModelReading.Observed ProjectModel.Observation.Unobserved
        | "registration-failed" ->
    ...
    test <@ ex.Message.Contains "no available project model" @>

[<Fact>]
let ``create REFUSES a model-unavailable outcome beside a model that WAS available`` () =
    let ex =
        Assert.Throws<ArgumentException>(fun () ->
            build (modelUnavailableSpec ProjectModelFixtures.available) |> ignore)

    test <@ ex.Message.Contains "MODEL-UNAVAILABLE verdict records an available project model" @>

[<Theory>]
[<InlineData(true)>]
[<InlineData(false)>]
let ``a model-unavailable verdict round-trips — outcome, exit code and the model state it names`` (observed: bool) =
    let model = if observed then rediscovering else notReported

    withTempDir "verdict-roundtrip" (fun root ->
        makeRepo root
        writeSpec root (modelUnavailableSpec model)

        let outcomeKind, modelSchema, modelStatus =
            use doc = JsonDocument.Parse(File.ReadAllText(Verdict.path root))
            let projectModel = doc.RootElement.GetProperty("projectModel")

            doc.RootElement.GetProperty("outcome").GetProperty("kind").GetString(),
            (match projectModel.TryGetProperty("schema") with
             | true, schema -> Some(schema.GetString())
             | false, _ -> None),
            projectModel.GetProperty("status").GetString()

        test <@ outcomeKind = "model-unavailable" @>

        // An observation is the daemon's own versioned payload; a missing one carries no
        // schema, so a consumer validating the schema cannot mistake it for an observation.
        if observed then
            test <@ modelSchema = Some "fshw-project-model-v1" @>
            test <@ modelStatus = "rediscovering" @>
        else
            test <@ modelSchema = None @>
            test <@ modelStatus = "not-reported" @>

        match Verdict.read root with
        | Verdict.Reading.Found v ->
            test <@ v.Outcome = Verdict.ModelUnavailable "NO VERDICT — PROJECT MODEL UNAVAILABLE" @>
            test <@ v.ExitCode = 2 @>
            test <@ v.ProjectModel = model @>
...
[<InlineData("not-reported-without-reason")>]
let ``a v2 verdict whose projectModel is absent or unreadable is Unreadable, never a green`` (shape: string) =
    let modelField =
        match shape with
        | "absent" -> ""
        | "malformed" -> ""","projectModel":{"schema":"fshw-project-model-v1","status":"available"}"""
        | _ -> ""","projectModel":{"status":"not-reported"}"""

    withTempDir "verdict-nomodel" (fun root ->
        makeRepo root
        Directory.CreateDirectory(FsHwPaths.root root) |> ignore

        File.WriteAllText(
            Verdict.path root,
            $$"""{"schema":"fshw-verdict-v2"{{modelField}},"treeHash":"sha256:x","outcome":{"kind":"green","baseline":{"kind":"full-suite-run","runId":"b0000000110040008000000000000110","earnedAt":"2026-09-06T12:00:00.0000000Z","projects":1} } }"""
        )

        match Verdict.read root with
        | Verdict.Reading.Unreadable reason -> test <@ reason.Contains "projectModel" @>
        | other -> failwith $"a verdict that does not say what model it was graded against is not one, got %A{other}")

[<Fact>]
let ``a hand-edited green beside a REDISCOVERING project model is refused on the way in`` () =
    withTempDir "verdict-edited" (fun root ->
        makeRepo root
        Directory.CreateDirectory(FsHwPaths.root root) |> ignore

        File.WriteAllText(
            Verdict.path root,
            """{"schema":"fshw-verdict-v2","projectModel":{"schema":"fshw-project-model-v1","status":"rediscovering","generation":4,"counts":null,"reasonCode":null},"treeHash":"sha256:x","outcome":{"kind":"green","baseline":{"kind":"full-suite-run","runId":"b0000000110040008000000000000110","earnedAt":"2026-09-06T12:00:00.0000000Z","projects":1}}}"""
        )

        match Verdict.read root with
        | Verdict.Reading.Unreadable reason -> test <@ reason.Contains "no available project model" @>
        | other -> failwith $"expected Unreadable, got %A{other}")

[<Fact>]
let ``a v1 verdict is Unreadable — it cannot say whether its green saw an available model`` () =
    withTempDir "verdict-v1" (fun root ->
        makeRepo root
        Directory.CreateDirectory(FsHwPaths.root root) |> ignore
        ...
        match Verdict.read root with
        | Verdict.Reading.Unreadable reason -> test <@ reason.Contains "fshw-verdict-v2" @>
        | other -> failwith $"expected Unreadable, got %A{other}")

[<Fact>]
let ``a terminal verdict records that no project model was read`` () =
    withTempDir "verdict-terminal" (fun root ->
        makeRepo root

        Verdict.writeHookFailure root [] Verdict.Check (Verdict.Invocation.start ()) [] [] "beforeRun refused"

        match Verdict.read root with
        | Verdict.Reading.Found v ->
            match v.ProjectModel with
            | ProjectModelReading.NotReported _ -> ()
            | other -> failwith $"a terminal verdict read no model, got %A{other}"
        | other -> failwith $"expected a readable terminal verdict, got %A{other}")

[<Fact>]
let ``a check-vs-confirm comparison never reads a model-unavailable run as an answer`` () =
    let reading outcome : Verdict.ImpactScopedRun =
        { Scope = ImpactFiltered(1, 2)
          Outcome = outcome
          FailingSuites = []
          Basis = Verdict.SampleBasis.Executed
    ...
    let unavailable = Verdict.ModelUnavailable "model gone"

    let earnedSide =
        Verdict.CheckComparison.ofRun (Some(reading (Verdict.Green BaselineFixtures.baseline))) unavailable

    let checkSide =
        Verdict.CheckComparison.ofRun (Some(reading unavailable)) (Verdict.Green BaselineFixtures.baseline)

    match earnedSide.Divergence, checkSide.Divergence with
    | Verdict.Divergence.Incomparable earned, Verdict.Divergence.Incomparable check ->
