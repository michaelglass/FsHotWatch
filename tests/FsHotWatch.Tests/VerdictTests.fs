...
let ``a changed CONTENT fixture makes the verdict stale — the dsa-scope-4 false green`` () =
    // exactly. `dsa-scope-4.json` changed (36 -> 40 leaf facts); MSBuild
...
let ``plugin verdicts carry outcome, elapsed and summary`` () =
    ...
    match Verdict.pluginVerdicts true statuses with
    | [ p ] ->
...
let ``a plugin that never ran is OMITTED, never invented as a pass`` () =
    ...
    test <@ List.isEmpty (Verdict.pluginVerdicts true (Map.ofList [ "coverage", idle ])) @>
...
let ``the verdict file and the agent status line resolve a plugin identically`` () =
    ...
    let fromFile = Verdict.pluginVerdicts true (Map.ofList [ "build", failed ])
...
let ``a running plugin is running; a timed-out one is timed-out, never merely failed`` () =
    ...
    test <@ Verdict.pluginOutcomeOf true running = Some Verdict.PluginOutcome.Running @>
    test <@ Verdict.pluginOutcomeOf true timedOut = Some Verdict.PluginOutcome.TimedOut @>
...
let ``warnings are a WARN when they fail the build, and invisible when they do not`` () =
    ...
    test <@ Verdict.pluginOutcomeOf true withWarnings = Some Verdict.PluginOutcome.Warn @>
    // Under `--no-warn-fail` it does not.
    test <@ Verdict.pluginOutcomeOf false withWarnings = Some Verdict.PluginOutcome.Ok @>
...
let ``an error-carrying plugin is a FAIL even when its run "completed"`` () =
    ...
    test <@ Verdict.pluginOutcomeOf true withErrors = Some Verdict.PluginOutcome.Fail @>
...
let ``an IDLE plugin is judged by the run it last completed`` () =
    ...
    test <@ Verdict.pluginOutcomeOf true idleAfterFailure = Some Verdict.PluginOutcome.Fail @>
    test <@ Verdict.pluginOutcomeOf true idleAfterTimeout = Some Verdict.PluginOutcome.TimedOut @>
    test <@ Verdict.pluginOutcomeOf true idleAfterPass = Some Verdict.PluginOutcome.Ok @>
...
// the signature — `started:` with no `elapsed:` — rebuilt inside the
...
// middle. (makes the same argument for the daemon handshake; this is
// its file-layer half.)
// ---------------------------------------------------------------------------
...
let ``two producers we could not read are NOT thereby the same producer`` () =
    let unknown =
        { Verdict.Binary = "(unknown)"
          Verdict.Hash = ContentHash.UnreadableSentinel }

    test <@ not (Verdict.Producer.same unknown unknown) @>

    let real =
        { Verdict.Binary = "a.dll"
          Verdict.Hash = "abc123" }
...
let ``the running fshw can identify itself`` () =
    ...
    test <@ ContentHash.isReadable me.Hash @>
