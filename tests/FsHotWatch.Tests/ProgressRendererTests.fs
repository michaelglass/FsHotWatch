...
/// reworked. The agent line has NO width budget, so it must not have a
...
// ---------------- a run that verified nothing is not a success ----------------
//
...
/// records for a run that executed no project. The summary is the
...
[<Fact(Timeout = 15000)>]
let ``compact renders a verified-nothing run as warn, never a check`` () =
    let lines =
...
[<Fact(Timeout = 15000)>]
let ``compact still renders a run that DID execute as a check`` () =
...
[<Fact(Timeout = 15000)>]
let ``verbose renders a verified-nothing run as warn, never a check`` () =
    let lines =
...
[<Fact(Timeout = 15000)>]
let ``verbose still renders a run that DID execute as a check`` () =
    // Positive control for the test above.
    let lines =
...
[<Fact(Timeout = 15000)>]
let ``agent tokens a verified-nothing run as warn, never ok`` () =
...
[<Fact(Timeout = 15000)>]
let ``agent still tokens a run that DID execute as ok`` () =
    // Positive control for the test above.
...
[<Fact(Timeout = 15000)>]
let ``a REPLAYED verified-nothing run is not a check either`` () =
...
[<Fact(Timeout = 15000)>]
let ``a verified-nothing record with no summary is worded from the case`` () =
...
[<Fact(Timeout = 15000)>]
let ``the glyph keys off the VerifiedNothing CASE, not the summary's words`` () =
...
[<Fact(Timeout = 15000)>]
let ``a verified-nothing run warns without reddening the verdict`` () =
...
[<Fact(Timeout = 15000)>]
let ``agent nextStep after a verified-nothing run points at status, never done`` () =
