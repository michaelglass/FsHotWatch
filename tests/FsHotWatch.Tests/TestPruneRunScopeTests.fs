/// Run scope: a run may clear only what it covered, the report evidence
/// that says what a run actually executed, and the stale-artifact preflight that settles
/// the ORDER in which staleness is asked about.
///
...
let ``a run that executed NO project records a verdict that says nothing was verified`` () =
    ...
    | PluginStatus.Completed(_, verdict) ->
        ...
        // and the host records the run as `RunOutcome.VerifiedNothing`.
...
[<InlineData(false)>]
[<InlineData(true)>]
let ``ordinary unchanged build preserves executed evidence through AlreadyVerified completion`` (filtered: bool) =
    withTempDir "build-noop-receipt" (fun root ->
        let config =
            ...
            if filtered then
                ...
                let narrow =
                    testsFinishedEvent
                        ...
                        (filteredLaunch [ "ProjA", [ "ProjATests" ] ] |> bindReceiptTree root)
        ...
        let settled =
            handler.Update ctx launched (Custom completion) |> Async.RunSynchronously
...
[<InlineData(false)>]
[<InlineData(true)>]
let ``unavailable execution revokes a previously earned receipt`` (artifactsUnavailable: bool) =
    withTempDir "unavailable-after-pass" (fun root ->
        let handler =
            create ":memory:" root (Some [ projConfig "ProjA" ]) None None None None []
...
[<Fact(Timeout = 20000)>]
let ``new dependency debt is not retired by unavailable execution after a pass`` () =
    withTempDir "new-dependency-debt" (fun root ->
        let handler =
            create ":memory:" root (Some [ projConfig "ProjA" ]) None None None None []
        ...
        let launched =
            handler.Update ctx withDebt (BuildCompleted BuildSucceeded)
            |> Async.RunSynchronously
        ...
        let unavailable =
            scheduled.Value(Invalid "dependency artifacts unavailable")
            |> Async.RunSynchronously
