...
let ``AUTOMATION-343: a cached test-prune replay clears the whole ledger, exactly as its real run does`` () =
    ...
    withTempDir "a343-tp-parity" (fun tmpDir ->
        // AUTOMATION-110: the cache key is salted by full-suite widening, and a repo with
...
[<InlineData(false)>]
[<InlineData(true)>]
let ``AUTOMATION-474 ordinary unchanged build preserves executed evidence through AlreadyVerified completion``
    (filtered: bool)
    =
