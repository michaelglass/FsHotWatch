...
[<Fact(Timeout = 15000)>]
[<Trait("SnapshotEvidence", "PendingDebtPublication")>]
let ``pending debt persistence follows the successful proposal and precedes acknowledgement`` () =
    ...
    let interrupted = create ":memory:" root (Some [ projConfig "ProjA" ]) None None None None []
    ...
    let published = create ":memory:" root (Some [ projConfig "ProjA" ]) None None None None []
...
let ``a failed command receipt acknowledges only its published owner outcome`` (invalidArtifacts: bool) =
    let handler = create ":memory:" (isolatedRoot ()) (Some [ projConfig "ProjA" ]) None None None None []
    let ctx, _, _ = makeTestPruneRecordingCtx ()
    let reply = System.Threading.Tasks.TaskCompletionSource<string>()
    let message =
