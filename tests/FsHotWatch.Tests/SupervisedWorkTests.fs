...
let ``owner transitions retain their result until actual store publication`` operation =
    let store = PluginWorkOwner.Store()
    let owner = PluginWorkOwner.Owner(0, store, "publication-boundary")
    let event, receipt = owner.AdmitTrackedEvent()
    ...
    let run =
        if operation = "complete-run" then
            ...
            owner.TryClaim "tests" |> Option.map fst
        else
            None

    let blocker =
        store.Register(
            { Name = "held publication writer"
              Value = box ()
              Busy = false
              Completed = 0L
              Failure = None
    ...
    use entered = new ManualResetEventSlim(false)
    use release = new ManualResetEventSlim(false)
    use attempted = new ManualResetEventSlim(false)
    ...
    let held =
        store.ChangeAsync(
            ...
            fun row ->
                entered.Set()
                Assert.True(release.Wait(TimeSpan.FromSeconds 15.0))
                row, ()
        )
    ...
    try
        Assert.True(attempted.Wait(TimeSpan.FromSeconds 5.0))

        Assert.False(
            transition.Wait(TimeSpan.FromSeconds 6.0),
            "a queued transition cannot report failure while its actual publication is still pending"
        )

        release.Set()
        awaitResult held
        Assert.True(transition.Wait(TimeSpan.FromSeconds 5.0))

        match transition.Result with
        | Result.Error error -> raise error
        | Result.Ok next ->
            match operation with
            | "publish" -> owner.SettleEvent(event)
            | "complete-run" -> owner.CommitEvent(next.Value, 2)
            | _ -> ()

        awaitResult receipt
        Assert.False(store.Snapshot.IsBusy)
        Assert.True(owner.Snapshot.Fault.IsNone)
        Assert.Equal((if operation = "complete-run" then 2 else 1), owner.Snapshot.State)
        Assert.Equal((if operation = "complete-run" then 2L else 1L), store.Snapshot.CompletedEvents)
    finally
        release.Set()
        awaitResult held
        Assert.True(transition.Wait(TimeSpan.FromSeconds 5.0), "owned transition task did not join")
