        ...
        let source =
            fsSource (
                ...
                @ [ // Filtering FOR the non-green — `recordRunOutcome`. Correct: it
                    ...
                    // control was written — the tracked issue made the real one
...
let ``runProcess reports TimedOut on kill, carrying the child's pre-kill stdout`` () =
    // This assertion USED to check that `partial` reached the tail.
    ...
    //
    // That was the tracked issue. It is fixed: the pumps own dedicated threads, so a
...
// ===========================================================================
// regression: the jj-merge wedge repro.
//
