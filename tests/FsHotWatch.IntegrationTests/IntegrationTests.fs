...
let ``Full pipeline: format → build → test`` () =
    ...
    // The formatter is the REPOSITORY's pinned tool: a repo without
...
let ``ProcessRegistry.killAll kills a child started via runProcess from another thread`` () =
    ...
    try
        ...
        | Error(:? IOException as error) -> Assert.Contains("without the required receipt", error.Message)
        | Error error -> Assert.Fail $"unexpected cancellation failure: {error}"
        | Ok outcome -> Assert.Fail $"intentional cancellation must not invent a target result: {outcome}"
...
[<Fact(Timeout = 180000)>]
let ``daemon stays responsive to status while mid auto-rebuild after a multi-file batch`` () =
