...
- **a run that verified nothing is a run-outcome CASE, not a summary
  ...
  payload carries it as `{"tag":"verifiedNothing","detail":…}`, and the compact, verbose,
  agent and `verdict.json` surfaces key their `⚠`/`warn` off the case. The status stays
  `Completed`, so exit codes are unchanged — a zero-project check still exits **3**
  (NO VERDICT), never 1. `RunSummary.saysNothingVerified` is gone (nothing parses the
  words any more); `RunSummary.nothingVerified` survives as the display formatter the
  verdict constructor uses. The task-cache entry format is bumped (4 → 5) because a
  cached verdict now records the fact and a pre-bump entry carrying the old marker would
  otherwise replay as a verified run: one cold re-run per repository.
...
- **the stale-artifact preflight no longer prescribes a command a
...
load file or assembly` on specific routes only, with the missing assembly sitting in the
output folder. Three occurrences in one day, hours each, and every time the identical tree
passed in a fresh workspace — because the symptom is indistinguishable from an application
bug.

The cause is an asymmetry between two artefacts derived from one restore. `dotnet restore`
writes `obj/project.assets.json`; only a BUILD writes `bin/<tfm>/<Asm>.deps.json` from it.
The deps-freshness gate repairs a stale restore by shelling `dotnet restore`, so it fixes
the compile — the `FS0074` error-storm disappears and reads as transient — and leaves the
runtime manifest listing the reference closure of a restore that no longer exists. The
host resolves assemblies through that manifest, never through the directory.

- The stale-artifact preflight gained a fourth case, `DepsManifestOlderThanRestore`,
  asked of the project about to run: is its `.deps.json` older than the
  `obj/project.assets.json` it is generated from? It **refuses** and names both files —
  the manifest is generated by an MSBuild target and nothing on disk holds the bytes that
  target would produce, so there is nothing honest to repair it with.
- The remedy names the two wrong answers as well as the right one. `dotnet restore` will
  not fix it (that is the whole defect), and adding a direct `ProjectReference` to
  whatever failed to load puts an entry in the manifest, makes the symptom vanish, and
  leaves the superseded restore exactly where it was.
- **Absence is never staleness, and equal is never stale.** Either half of the pair
  missing means there is nothing to compare — an old-style project, a cleaned `obj/`, a
  library that generates no manifest. A build writes the restore first and the manifest
  after it, so equal timestamps are the normal healthy shape.
- **A matching dep-content baseline can no longer certify a MISSING restore.**
  `evaluateProject` suppresses a phantom `Stale` when the dep-relevant signature matches
  the last known-good one. Nothing in that signature comes from `obj/`, so an ordinary
  `rm -rf obj/` left the signature unchanged and the project was certified fresh with no
  restore output on disk at all — then type-checked against an empty reference set. The
  suppression now requires the assets to actually be present.
- ADR-017 records the rule the two freshness subsystems were disagreeing over. `mtime` is
  correct exactly when the two files share no bytes and the question is temporal (a source
  and the assembly compiled from it; a restore and the manifest generated from it); content
...
remains is a spurious restore chain, not a wedge.

- **a check's verdict accounts for every test run it produced, not
...
### test-prune: an observation is not a debt — a phantom obligation stops costing a whole suite
...
- **a verdict read while a run is in flight can no longer return the
...
- **a broadly red suite no longer kills the check that ran it.** The
...
- **a cold or forced scan is no longer mistaken for idleness — the
...
- **the cold-scan retry bound has a named outcome, and every scan
...
- **Case 2:** `fshw invalidate` clears every cached plugin task
...
- **** coverage-derived source-file → test-project evidence is
...
explicitly limited to observed failures—not general relevance recall.
...
### release: the publication barrier waits long enough for nuget.org to index
...
### release: the publication barrier verifies a dotnet TOOL by installing and running it
...
### repo tasks: every daemon-backed `mise` task materializes every configured analyzer
...
### format: the plugin runs the Fantomas the repository pins, and says so
...
weakened it certifies the tree afterwards.
...
### core/build: MSBuild's own generated `obj/` sources stop reading as edits
...
- **cli** — the daemon and `--run-once` paths each carried the same six-arm match over
  `CheckOutcome`, so both had to add their arms twice.
...
Measured against a real recreate rather than a simulated one: the
...
one layer up: **a ledger you could not read is not an empty ledger.**
...
### `fshw --version` names the source ref it was built from
...
handshake and the verdict's producer hash:
...
### `confirm` on a warm cache: a refusal despite evidence
...
`.fshw.json`, so `confirm` does not run it. It does **not** claim to run
