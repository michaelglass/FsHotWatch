...
- AUTOMATION-339: **a run that verified nothing is a run-outcome CASE, not a summary
  string.** AUTOMATION-198 stopped a zero-project test run from rendering `✓`, but did it
...
### test-prune: a superseded restore is named before the suite runs, not after a route dies (AUTOMATION-528)
...
Also recorded there: AUTOMATION-538's mirror report — a content-identical touch of a
