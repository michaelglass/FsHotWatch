...
/// object during it. Sizes are summed per type from the walk; the GC's own
/// `GCHeapStats` and `GC/CommittedUsage` for that same collection, and the System.Runtime
/// `gc-committed` counter just after it, are captured alongside for reconciliation.
let walkHeap (port: string) (timeout: TimeSpan) : Result<HeapWalk, string> =
    try
        let client =
        ...
        // Through the DYNAMIC parser: neither event has a typed TraceEvent parser, and an
        // event no parser claims arrives undecoded (no name, no payload).
        ...
        while (countersAfter < 2 || heapStats.IsNone)
              && DateTime.UtcNow < deadline
              && not processing.IsCompleted do
            Thread.Sleep 100

            if not (isNull (Environment.GetEnvironmentVariable "FSHW_BENCH_DEBUG")) then
                eprintfn
                    "[walk] gc=%d walkGc=%d done=%b nodes=%d stats=%b committed=%A counters=%d"
                    currentGc
                    walkGc
                    walkDone
                    bytes.Count
        ...
        processing.Wait(TimeSpan.FromMinutes 10.0) |> ignore
