...
/// The version the working CLI shipped, per the package diff. The
...
let private syntheticManifest (libraries: (string * string) list) : string =
    ...
    let path = Path.Combine(Path.GetTempPath(), $"fshw-control-%s{unique}.json")
...
printfn "Checking app-local MSBuild support assemblies..."
