...
qualified together with the release: consumers that accept only the v1 schema
will reject the new envelope until their compatibility handling is updated.
...
passed, 17 failed, zero skipped. `/tmp/a104-integrated-unit-red.log` records both
...
remain coordinated follow-ups. Verdict-v2 adapter compatibility remains part of
the release work; no supported consumer pin has been claimed.
