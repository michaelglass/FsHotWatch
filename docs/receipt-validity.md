...
Workspace: /Users/michaelglass/Developer/opensource/FsHotWatch/.workspaces/receipt-loss
...
Overlap: edits TestPrunePlugin and CLI verdict-cause/reporting files; coordinate exact hunk ownership before production work, avoid its workspace. Identity/cache work is untouched and must remain independent. Runtime-debt lifecycle shares TestPrunePlugin but is separate behavior; new-debt control must preserve its generation retirement semantics. Supervision/work-ledger architecture is not required for this narrow regression.
