...
2. **Measuring perturbed the thing measured.** The `fshw test-rerun` calls the
   ...
   stamp a content-free green. The act of measuring created the
...
3. **A verdict could not say which tree it was about.** A changed JSON fixture
   ...
   The green was real. It was just a green about a different tree.
...
- `check` vs `confirm` (named `gate` until): the verb names its guarantee.
- identity derived from content, never asserted by a label. The
