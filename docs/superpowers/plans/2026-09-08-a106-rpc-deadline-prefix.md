# A106 RPC synchronous-prefix deadline implementation plan
...
**Architecture:** Start the existing finite timer before scheduling the complete callback on the task pool. Await the unwrapped callback task through the existing deadline/watchdog bracket. This is one independently testable outer-guard correction within A104/A106, not completion of their shared PluginCore/WorkLedger design.

**Tech Stack:** F#, .NET Task, existing StreamJsonRpc adapter and xUnit tests.

**Spec:** AUTOMATION-104 and AUTOMATION-106; full fetched descriptions retained at /private/tmp/a104-current-description.txt and /private/tmp/a106-current-description.txt. Source mapping: docs/automation-104-106-source-preparation.md.
...
- Keep A104/A106 In Progress. Do not merge or claim their full acceptance from this test class.
- No changes to A523's discovery files, no dependency changes, no daemon restart.
...
- [x] Save exact receipts and describe the WIP with an explicit jj message. Preserve the still-red A104 snapshot regression and remaining full design scope.
